...
src/
  ...
  browser/
    ...
    features/                 機能。**機能どうしは import しない**
      ...
      screen-nav/             全画面の最上部の帯。3画面の口と答え待ちの印（13.9）
...
└ <TurnSelectionProvider>    選んでいるターンを配る（6.2）
   └ <Root>                  main.tsx の中（export しない）。useScreen() で出す画面を選ぶ（6.2・13.6）。
      ...
      ├ <ScreenNav>          **全画面の最上部の帯**（13.9）。3つの口（会話 / キャラクター /
      │                      トークン消費）と答え待ちの印。狭い画面ではタブ帯の右端の「≡」に畳む
...
- **帯に出すのは画面の口と答え待ちの印だけ。** 設定の操作子は置かない（13.6 の割り方を変えない）
