...
src/
  ...
  browser/
    ...
    components/               React の部品。**`app/` `page/` `domain/` `ui/` の4段**（2026-09-25・2026-09-26 決定。下の箱の表）
      app/                    root.tsx（出す画面を選ぶ <Root>。すべての画面を知る composition root）
...
```
components/page/<ページ>/
  <ページ>.tsx                   container（入口。components/app/root.tsx が置く）。名前は Screen の値
...
- **部品のディレクトリの外から引いてよいのは `<部品>.tsx` だけ**（中の `hooks/` `domain/`
  ...
  外に置くとき（書き終わりの知らせ `DiaryNotice`）は、`components/app/root.tsx` がその `<部品>.tsx` を直に
...
| `components/app/`      | **出す画面を選ぶ `<Root>`**。すべての画面を知る composition root                                          | `components/page` / `components/domain` / `components/ui` / `features` / `hooks` / `domain` / `lib` / `utils` / `stores` / `shared` |
...
- **親が子を組む形も領域どうしの import に数える。** `<Layout>` は領域の中身を props で
  受け取るだけで他の領域を知らず、**どの画面を出すかは `components/app/root.tsx` の `<Root>` が選ぶ**
...
| -------------------------- | ------------------------------------------------------------------ | ------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| **枠**（frame）            | 全画面で共有する枠。差し込み口は props で受け、画面を知らない      | `components/domain/<枠>/` | 画面と `<Root>` から import してよい。**枠どうしは import しない**                                         | `layout` / `screen-nav` / `sidebar`                          |
| **画面**（screen）         | 1つの画面 = 1つのページ。**どの画面を出すかを決めるのは `<Root>`** | `components/page/<画面>/` | `<Root>` だけが import する。**画面どうしは import しない**                                                | `conversation` / `character` / `token-usage` / `achievement` |
...
名指しすると、その場面を `opening` の続きとして流し、次の `prompt()` はその次の
場面から続く。**`opening` と名指しの場面は、最初のタブが繋がってから流し始める**
（`src/session-start.ts` が `subscribe` を包んで知らせる。プランと effort の対応だけは起こした直後）。
起こした直後から流すと、ページが繋がる前に届いたぶんが `hello` の状態に畳まれ、どこからが
`events` として届くかが開くまでの時間で変わる（E2E のメッセージの列が走らせるたびに揃わない）。**依頼を手で送らずに特定の状態を出す**ための口で、状態のカタログを撮る
...
| `TSUKUMO_FAKE_SCENE`              | `fake` のとき最初のタブが繋がったら流す場面の名前                                              | 流さない         |
...
| `TSUKUMO_FIXED_CLOCK`             | サーバの時計をその瞬間（ISO 8601）で凍らせる（10章「E2E の成果物と再現」）                     | 本物の時計       |
...
└ <TurnSelectionProvider>    選んでいるターンを配る（6.2）
   └ <Root>                  components/app/root.tsx。useScreen() で出す画面を選ぶ（6.2・13.6）。
...
**部品の置き場**（2章「`src/browser/` の箱と、置く基準」）: Provider は `app.tsx` の `<App>`、出す画面を選ぶ `<Root>` は `components/app/root.tsx`（入口の `main.tsx` は `<App>` を mount するだけ）。
...
- **ほかの記録は、一覧から名前ごと消えるとき（`"delete"`）だけ一緒に消す。** あらすじ
  （`~/.tsukumo/chat-summary/<name>.md`）とアーカイブ（`~/.tsukumo/chat-archive/<name>/`。日ごとの
  会話・エピソード索引・思い出した記録。組み替える前の旗と索引のファイルも）はパックの外にあり、
  **鍵がパックの名前だけ**なので、残すと同じ id で作り直したパックが消したパックのあらすじを載せ、会話を読み戻して思い出す。作り直しのときに
...
2026-09-21 決定。2026-09-25 に書き手を `/compact` の写しから定着の `query()` へ替えた。
**`~/.tsukumo/chat-summary/<パック名>.md` に、あらすじを1つだけ置く**（認められた例外の範囲は
`docs/chat-mode.md` 4.9「記憶の圧縮と忘却」が正典。用語は `docs/glossary.md`「あらすじ」で、
識別子は組み替える前の `chatSummary` のまま）。ここが決めるのは**どこに・誰が・どう書き、
どう渡すか**と上限。
...
- **キャラクターパックのディレクトリの中には置かない。** パックは素材のまとまりで、ホームへ丸ごと
  ...
  **パックを渡すことがあらすじを渡すことになる**
- **エピソード索引の隣（`chat-archive/<パック名>/`）へ移さない。** 置き場を変えると、組み替える前の
  写しを種に読む口と、印を移し替える手順が別に要る。**同じファイルのまま書き手だけを替えれば、
  はじめの定着が「前のあらすじ」として写しを読み、それを上書きする**ので、種に使うのは構造上
  1回だけになる（`docs/chat-mode.md` 4.9）
- パスに使う文字列はパックの名前1つだけで、`isCharacterPackName` を通してから組み立てる
  （`..` も区切り文字も名前として通らない。7.1 と同じ規則）
- 中身は**1行目が印、2行目からあらすじの本文**（見出しも日時も付けない）。印は「次に起こす
  セッションへ渡す必要があるか」の1ビットで、下の「渡し方」が使う。**同じファイルに持つ**のは、
  あらすじと印が1回の書き込みで揃うから（別ファイルにすると片方だけ残った状態が生まれる）。
  ファイルを消せば印も一緒に消える（元に戻す手が1つのまま）。`systemPrompt` へ載せるときの
  前置きは core 側で、**印の行は載せない**

**書くのは定着**（下の「定着はどこで走るか」）。定着の `query()` が返したあらすじと話題の見出しを、
tsukumo が1つの本文に組んで上書きする。**書いたときの印は「渡し済み」**（いま動いている
セッションは、畳んだ会話をもう文脈に持っている）。

- **`PostCompact` フックで写すのはやめる。** tsukumo は `/compact` を投げない。利用者が手で
  `/compact` を叩いても、claude 自身の自動の圧縮が起きても、あらすじは書き換わらない
  （`docs/chat-mode.md` 4.9）
- 受け取った文字列は**ログにも画面にも出さない**（出してよいのは長さまで）
- **`/clear` は今までどおり印で受ける。** `conversation-cleared`（`src/server/session-driver/core/sdk-message.ts`）が
  流れたら、印を「未渡し」に戻す。見る場所は `relayMessages` の中で、`turn-finished` で
  `personaMemory.finishTurn()` を呼んでいるのと同じ1行の形

**最近の話題の見出しも同じファイルから取る**（2026-09-23 決定。範囲と理由は
`docs/chat-mode.md` 4.9「記憶の圧縮と忘却」。画面は 13.7「雑談のときのサイドバー」）。
別のファイルは持たない。

- **書き方**: 定着の出力の `topics`（新しい順に3件まで）を、tsukumo があらすじの本文の
  **いちばん最後**に `<topics>` と `</topics>` の行で挟んで置く（1行1件、`- ` で始める）。
  **モデルに組を書かせず tsukumo が組む**ので、下書きに組が紛れることは無いが、取り出し方は
...
- **流す契機は2つ**で、どちらも `chat-topics-changed` イベント（`SessionState.chatTopics` に
  ...
  書いたあと**（**書いた直後のファイルを読み直して**流す。上限で行が落ちたあとのファイルから
  取るので、次に起こしたときと同じ見出しになる）。読んで見出しにするのは core の
  `readChatTopics` 1つで、2つの契機はそれを呼ぶだけ
- **運ぶのは見出しだけ**で、あらすじの本文はブラウザへ渡らない

**渡し方は `systemPrompt` の append**（`persona.md` と同じ道。`takeSystemPromptAppend`）。
**同じ口で直近の逐語（作業記憶）も渡す**（`docs/chat-mode.md` 4.9「直近の会話は逐語のまま
読み戻す」）。載せるかどうかの条件は下の2つで**あらすじと逐語に共通**なので、**判断は1箇所に
まとめる**。

- **載せる条件は2つで、どちらかに当たれば載せる**: (1) 続きから始めない（`SessionLaunchSeed.resume`
  が `undefined`） (2) 続きから始めるが、**印が「未渡し」**。どちらでもなければ載せない
  （`resume` した文脈に同じ会話が既にある）。**判断そのものは core の中で閉じる**
...
- **印が無い・読めないときは「未渡し」として扱う。** 倒れる方向を「同じあらすじが2度載る」側に
  して、**黙って記憶が消えるほうへ倒さない**
- **載せたら印を「渡し済み」に戻す**（同じあらすじを起こし直しのたびに重ねない）
- **雑談のときだけ**渡す（`personaMemory` と同じ単位。`system-prompt.ts` が規約を選ぶのと同じ境目）
- 前置きの文面は core（`chat-manner.ts` の隣）。**あらすじであって会話ではないこと**と、
  **引用しないこと**を短く添える
- **判断と印の書き換えを1回で済ませ、あらすじと直近の逐語を同じ機会に組み立てて返す**
  （口を2つに分けない。**採らなかった案は `docs/history/decision.md`「design.md 7. 雑談の記憶の
  要約はどこに置くか（覆した記録と採らない案）」**）。**並びはあらすじ → 直近の逐語の2節**
  （組み替える前にあった旗の節は無くなる）
- **逐語だけが載る場合がある。** 条件を決めるのは `resume` と印だけなので、**あらすじがまだ無い
  （一度も定着していない）ときも逐語は載る**
- ファイル名は `chat-memory-prompt.ts`（概念は**「雑談の記憶を `systemPrompt` に載せる」**。原則5）
...
| --------------- | ------------------------------------------------------------------------------------------------------------------- |
| 積み重ね方      | **上書き**（継ぎ足さない）                                                                                          |
| 1ファイルの上限 | 容量の表の `synopsisBytes`（**8 KiB**。印の行を含める）。溢れたら**古いほうの行から落とす**（行の途中では切らない） |
| 持つ世代        | 1つだけ（バックアップを取らない）                                                                                   |
| 触るファイル    | そのパックの1つだけ                                                                                                 |

- **継ぎ足さない理由**: 定着は前のあらすじを入力に受け取って書き直すので、継ぎ足すと同じ内容が
  二重になる
- **上限の根拠は `docs/chat-mode.md` 4.9 の容量の表**（要約が会話の写しに育つ経路を塞ぐ）。
  **作業記憶を広げてもここは連れて動かさない**（目的が別）
...
触るのは `adapter`（`chat/adapter/chat-summary.ts`）、結ぶのは配線層（`src/session-start.ts`）。

**`~/.tsukumo/` の中身のうち雑談の記憶は、あらすじ（`chat-summary/`）と雑談の会話のアーカイブ
（`chat-archive/`。エピソード索引を含む。下）の2つ**（9章）。
...
- **`state.records` からは書かない。** `trimToRecentTurns` に切り詰められたあとの記録から
  拾うと、窓から溢れたターンが永久に落ちる（`docs/chat-mode.md` 4.9）。**書く材料は届いた
  イベントそのもの**
...
- **窓から溢れた会話は、この口では返さない。** 定着で畳まれてエピソード索引から引く（下の
  「エピソード索引はどこに置くか」）。組み替える前にこの口が一緒に返していた旗のぶん
  （`kept`）は無くなり、`limits` は窓の量1つになる
...
2026-09-25 決定。**何を・いつ畳むかの正典は `docs/chat-mode.md` 4.9「窓から溢れた会話は定着で
畳む」**（窓から溢れた未定着の行・ターンの終わり・同時に1本・1回に契機の2倍まで）。ここが決めるのは
**どのファイルが何を持ち、`query()` に何を渡して何を受け取るか**。

**「残す」旗（`keep` ツール・`ChatKeep`・`kept.jsonl`）と、キャラクターが書く索引（`index` ツール・
`index.jsonl`）はやめる**（組み替える前の形は `docs/history/decision.md` と git の履歴にある）。
**どちらのファイルも読まなくなり、消さずに残す。**

| ファイル                                            | 持ち物                                                                                                                                                               |
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `src/server/chat/core/chat-consolidation.ts`        | 指示文・依頼の文面の組み立て・出力の形（JSON Schema）・出力の検査・話題の組（`<topics>`）の書き方と取り出し方・モデルと時間切れ（純関数と定数）                      |
| `src/server/chat/core/chat-consolidation-writer.ts` | 1回ぶんの流れ（未定着の行を数えて契機に届いていれば起こす・時間切れ・検査・エピソード → あらすじの順に書く・書いたファイルから話題を読み直す）。決して reject しない |
| `src/server/chat/adapter/sdk-chat-consolidation.ts` | 使い捨ての `query()`（`structured_output` を返すだけ。原則3。形は `sdk-visit-script.ts` に揃える）                                                                   |
| `src/server/chat/adapter/chat-archive.ts`           | 未定着の行の取り出し・エピソードの追記（下の「エピソード索引はどこに置くか」）                                                                                       |
| `src/server/chat/adapter/chat-summary.ts`           | あらすじと印の読み書き（上の「雑談の記憶の要約はどこに置くか」）                                                                                                     |
| `src/server/session/core/session-manager.ts`        | ターンの終わりの契機（雑談のときだけ、走っていなければ1本起こす。数えるのは書き手）。**待たずに次へ進む**                                                            |

**起こし方**（訪問の台本と同じ。5章「訪問の台本」）: 軽いモデル（`CHAT_CONSOLIDATION_MODEL`。
はじめは `haiku`）・`tools: []`・`mcpServers: {}`・`settingSources: []`・`maxTurns: 1`・
`persistSession: false`・考える段なし（`thinking: disabled`）・`outputFormat: { type: "json_schema" }`。
指示文は `systemPrompt` をそのまま置き換える（Claude Code の既定の指示文も CLAUDE.md も載らない）。
**時間切れは 120 秒**（日記の問い合わせと同じ。背景で走り、利用者を待たせない）。

**渡すもの**（依頼の文面。組むのは core）:

- **畳む行**: 行番号（1始まり）・話者の別・文面の並び。日付が変わるところに見出しを1つ挟む。
  **表情・画像の枚数・時刻は渡さない**（作業記憶と同じ線）
- **前のあらすじ**（無ければ空）。本文だけで、印の行と `<topics>` の組は渡さない
- **直前のエピソードの見出し**（無ければ無し）。話題が続いているかを見分けるため

**受け取る形**（`structured_output`）:
...
| 鍵         | 決め                                                                                                                      |
| ---------- | ------------------------------------------------------------------------------------------------------------------------- |
| `episodes` | 1件以上。**`end` はそのエピソードの最後の行番号**で、前のものより大きく、**最後のものは入力の最後の行**（切れ目なく覆う） |
| `title`    | 空でない 20 文字まで・改行を含まない                                                                                      |
| `gist`     | 空でない 120 文字まで（`remember` の1行と同じ値）                                                                         |
| `cues`     | 1〜10 個・各 20 文字まで。言い換え・上位の語を含める                                                                      |
| `weight`   | 1・2・3 の整数（目安は `docs/chat-mode.md` 4.9）                                                                          |
| `synopsis` | 空でもよい（`<topics>` の組は含めない。tsukumo が組む）                                                                   |
| `topics`   | 0〜3 個・各 40 文字まで                                                                                                   |

- **区切りを行番号で返させる**のは、モデルに時刻を写させないため。行番号から各エピソードの
  始まりと終わりの**アーカイブの行の時刻**（`from` / `to`）に直すのは tsukumo
- **検査は丸ごと**（`parseVisitScript` と同じ）。1か所でも崩れていたらその回は何も書かず、行は
  未定着のまま残る。長すぎるあらすじは落とさず、書くときに上限で古い行から落とす
- **書く順はエピソード → あらすじ**。途中で止まったときに「エピソードはあるがあらすじが古い」側に
  倒す（次の定着が新しいエピソードの要旨を踏まえられないだけで、行は二重に畳まれない）
- **話題が前の回から続いていても、前のエピソードは書き換えない**（追記だけ）。続きは新しい
  エピソードになり、見出しが似るのは許す

**同時に1本・契機は捨てる**の持ち場は `session-manager`（走っているかどうかの1ビットを持つ）。
**数えるのは1ターンに1回**（ターンの終わり）で、走り終えても次のターンの終わりまで次を起こさない。
**次のターン・パックの切り替え・プロセスの終わりと重なっても待たない** — 書きかけで止まれば、
追記済みのエピソードまでが定着したことになり、残りは次の契機で拾い直す。**走っているかどうかの
1ビットは駆動の代ではなく `session-manager` 自身が持つ**（起こし直しでは代だけを作り直すので、
同じパックへ起こし直した直後に同じ行を2本で畳まない）。起こし直しのあとで結果が届いても
**書くのは起こした時点のパックのファイル**で、話題の見出しを流すのは、そのときの状態が雑談で
同じパックのときだけ。プロセスを終えるときは走っている1本を中断し、中断のあとに届いた結果は
書かない。

#### エピソード索引はどこに置くか

2026-09-25 決定。**持つかどうか・どう引くかの正典は `docs/chat-mode.md` 4.9「古い雑談は索引を
引いて思い出す」**（2段階・採点の4要素）。ここが決めるのは**どこに・どんな形で書き、どう読むか**
と、**容量と係数の表の置き場**。

**置き場は `~/.tsukumo/chat-archive/<パック名>/episode.jsonl` と `recalled.jsonl`。**

- **アーカイブと同じディレクトリ**（索引はそのパックの会話の目次で、別の親に分ける理由が無い）。
  **パックをまたがない**。キャラクターを消すとディレクトリごと消える（7.1「消すときの細部」）
- **日付のファイルとは名前で見分ける。** 窓の側は `YYYY-MM-DD.jsonl` にだけ当たる正規表現で
  ファイルを選ぶので、2つとも**窓の走査に混ざらない**
- **日ごとに割らない。** エピソードは定着1回に数件で、太り方は日数で読める

**`episode.jsonl` の形式は JSONL（1行1件、追記）。** 書くのは tsukumo（中身は定着の出力）。

```jsonl
{
  "v": 2,
  "id": "2026-09-25-3",
  "from": "2026-09-25T14:03:12+09:00",
  "to": "2026-09-25T14:40:05+09:00",
  "title": "（見出し）",
  "gist": "（要旨）",
  "cues": [
    "（手がかり語）"
...
| 鍵            | 中身                                                                                              |
| ------------- | ------------------------------------------------------------------------------------------------- |
| `v`           | 行の形の版。**`2`**（アーカイブの行の `1` と見分ける。`index.jsonl` の `1` とも）                 |
| `id`          | `to` のローカル日付＋その日の通し番号（`-1` から）。tsukumo が振る                                |
| `from` / `to` | そのエピソードの最初と最後の**アーカイブの行の `at`**。**逐語はアーカイブにしか無く、索引は目次** |
| `title`       | 見出し（20 文字まで）                                                                             |
| `gist`        | 要旨（120 文字まで）                                                                              |
| `cues`        | 手がかり語（10 個まで）                                                                           |
| `weight`      | 大事さ 1〜3                                                                                       |

- **どこまで定着したかは、最後の行の `to` から分かる**（別の印を持たない）。未定着の行は
  「`to` より後の `at` を持ち、作業記憶の窓（新しいほうから `recentBytes`）の外にある行」。
  **同じ秒に書かれた行は区別しない**（`to` と同じ秒の後続の行は定着済みと読まれうる。**会話は
  アーカイブに残る**ので、落ちるのは索引から引けないことだけ。秒より細かい印を足すほどの害ではない）
- **読めない行は飛ばす**（壊れた JSON・知らない版）。例外は投げない（索引が無い・壊れていても
  `readRecent` はそのまま動く。無ければ次の定着がアーカイブの最初から畳む）

**`recalled.jsonl` は思い出した記録**（`{"v":1,"id":"…","at":"…"}` を、`recall_episode` で1件
開くたびに追記）。**索引の行を書き換えないために別に持つ**（追記だけなら途中で止まっても被害が
1行で済む）。文面を持たないので、会話の複製に数えない。

**読む口**（`ChatArchive` に足す。型は `session-driver/core/session-driver.ts`、実装は
`chat/adapter/chat-archive.ts`。**ファイルに触るのは同じ1ファイルのまま**）:

```
unconsolidated(packName, { recentBytes, maxBytes }) → 未定着の行（古い→新しい。行番号を振る元）
appendEpisodes(packName, episodes)                   → 追記（id を振る）
recallList(packName, words, limitBytes, now)         → 候補の一覧（採点の高い順）| 当たらない
recallEpisode(packName, id, limitBytes)              → その範囲の逐語（古いほうから）| 無い
```

- **採点は `chat/core/` の純関数**（アダプタは索引と記録を読んで渡すだけ）。1ターンの回数の
  上限（`recallListsPerTurn` / `recallEpisodesPerTurn`）を数えるのも core で、ターンの終わりの合図は
  今の `finishTurn` に相乗りする
- **一覧に返すのは `id`・`title`・`gist` だけ**。`recallListBytes` に収まるところで止める
  （1件を単位にし、溢れる1件は載せない）
- **開いた1件の返す形は `readRecent` と同じ**（話者の別・文面・その行の日付だけ）。モデルへ返す
  文面に組むのは core（`chat/core/chat-memory-prompt.ts`）で、**逐語の並べ方は `systemPrompt` の
  節と同じ1つ**
- **`grep` を起こさない。** `node:fs` で読んで TypeScript で計算する（外部コマンドを増やすには
  ユーザーの承認が要る）

**容量の表と採点の表は `src/shared/chat-memory-budget.ts` の1ファイルに置く。** サーバの複数の
機能（`session`・`system-prompt`・`chat`）と配線が読み、組み替える前の定数も `shared` にあった。
**値の根拠は `docs/chat-mode.md` 4.9 にだけ書き、doc コメントはそこを指す。**
...
- **`src/shared/chat-log.ts` の容量の定数4つはこの表へ移す。** 畳む閾値（`CHAT_COMPACT_THRESHOLD_BYTES`）と
  旗（`CHAT_KEPT_READBACK_BYTES`）は `/compact` と `keep` と一緒に消え、窓と `recall` の値は表の
  `recentBytes` と `recallEpisodeBytes` になる
- **表から導く値は定数にしない**（定着の1回の入力の上限は `consolidateEveryBytes * 2`）

**採点の式**（引く言葉は空白で分けた語の並び。比べる前に小文字にする）:

| 段     | 式                                                                                                                                                                        |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 一致   | 語ごとに、手がかり語・見出し・要旨それぞれへの当たり（含まれれば 1、でなければ**語の2文字ずつの組のうち相手に含まれる割合**）に欄の重みを掛けた最大をとり、語について足す |
| 足切り | 一致が `minMatch` 未満のエピソードは候補にしない                                                                                                                          |
| 新しさ | `max(recencyFloor, 0.5 ^ (to からの日数 / (halfLifeDays × (1 + halfLifeGainPerRecall × 思い出した回数))))`                                                                |
| 点     | 一致 × `weight` × 新しさ。高い順に並べ、同点は新しい順                                                                                                                    |

- **係数は目安**で、使ってみて表の値だけを直す（式を直すのはそれで足りないと分かってから）
- **1文字の語**は2文字の組が作れないので、含まれるかどうかだけで見る

**上限と消す手**

| 何           | 決め                                                                                                        |
| ------------ | ----------------------------------------------------------------------------------------------------------- |
| 積み重ね方   | **追記**（2つとも1行1件、上書きしない）                                                                     |
| 索引の上限   | **持たない**（読むのは採点のためだけで、返す量は `recallListBytes` と `recallEpisodeBytes` で頭打ち）       |
| 消す手       | `episode.jsonl` を消す（次の定着が最初から畳み直す）・`recalled.jsonl` を消す（思い出した回数だけが落ちる） |
...
- 場面は `TSUKUMO_FAKE_SCENE` で名指しするか、入力欄から依頼を送って次の場面を流す。
  **名指しの場面も `opening` も、ページが繋がってから流れ始める**（5章「fake-driver.ts」。
