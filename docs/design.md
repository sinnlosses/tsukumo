...
「`src/browser/styles/` に集め、機能と同居させない」を置き換えた）。置き場は**機能ごとに1枚**
...
- **キャラクターごとに別のセッションを持つ**（2026-09-14 決定。「キャラクターごとに別の部屋が
  ある」）。セッションの印を **`tsukumo:<パック名>`**（雑談は **`tsukumo:<パック名>:chat`**。
  13.7）にし、**起動時も切り替え時も、これから起こす側の印を持つ最新のセッションを探して
  `resume` する**（無ければ新規）。印の組み立ては `core/config.ts` の `sessionTag` 1箇所で、
...
- **背景（13.8）もこの経路に乗り、上限は据え置く**（2026-09-21 決定）。**枚数は 8 のまま**で
  足りる（表情6 + ミニ立ち絵1 + 背景1 = 8）。受け付けるのは `.png` / `.jpg` / `.webp` の3つ
...
残る場所が増える（9章）。**画面から消す口もいまは作らない**（対象利用者は作者本人だけで、
ファイルを直接開ける）。

#### 雑談の記憶の要約はどこに置くか

2026-09-21 決定。**`~/.tsukumo/chat-summary/<パック名>.md` に1つだけ置く**（同じ日に一度
「置かない」と決めたが**覆した**。覆した理由と、認められた例外の範囲は
`docs/requirements.md` 4.9「記憶の圧縮と忘却」が正典）。ここが決めるのは**どこに・誰が・どう書き、
どう渡すか**と上限。
...
- **8 KiB は閾値（32 KiB）の 1/4。** 要約がそこへ近づいたときは、畳まれたものではなく
  **要約の形をした会話の写し**になっている。そこで切る
...
| 読み戻す量 | **16 KiB**（文面のバイト数。`docs/requirements.md` 4.9） |
...
| 雑談の記憶の要約                     | **新規**。作るのは claude 自身の圧縮（`/compact`）で、tsukumo がするのは容量を数えて圧縮を頼むことと、区切りを画面に出すことだけ。**要約の文面は画面にも `error` フレームにも stderr にも出さない**                                                                                                                                                                                                                                                                                                                           |
| 雑談の要約の写し                     | **新規**。`~/.tsukumo/chat-summary/<pack>.md` に**最新の1つだけ**を上書きで持つ（8 KiB まで）。**ユーザーが 2026-09-21 に認めた「別の場所に複製しない」の例外の1つ目**（範囲と理由は `docs/requirements.md` 4.9、形と上限は7章）。載せ直すのは**雑談のセッションの `systemPrompt`** で、条件は「新規に起こした」か「`/clear` を見たあと」の2つ（写しの1行目の印が持つ）                                                                                                                                                       |
| 雑談の会話のアーカイブ               | **新規**。`~/.tsukumo/chat-archive/<pack>/<日付>.jsonl` に、雑談の依頼とセリフを表情つきで1行ずつ追記する。**ユーザーが 2026-09-21 に認めた「別の場所に複製しない」の例外の2つ目**（範囲と理由は `docs/requirements.md` 4.9、形と上限は7章）。**画面の 100 ターンには影響されない。** 画面にも `error` フレームにも stderr にも出さない                                                                                                                                                                                       |
| 直近の雑談を逐語で読み戻す           | **新規**（2026-09-21。**同じ日にここへ書いた「claude へ戻すこともしない」を覆した**。ユーザーの承認による）。アーカイブの**新しいほうから 16 KiB まで**を読み、**雑談のセッションの `systemPrompt`** へ逐語のまま載せる。載せる条件は要約の写しと同じ2つ。**渡す先はそこだけ**で、画面にも `error` フレームにも stderr にも出さず、**仕事の側の文脈にも載せない**。逐語が新しいセッションの transcript に書かれることは承認に含まれる（範囲と量は `docs/requirements.md` 4.9「直近の会話は逐語のまま読み戻す」、読み口は7章） |
| 人格への書き戻し（覚えたこと）       | **新規**。雑談で覚えたことを `~/.tsukumo/characters/<pack>/persona.md` の末尾の節へ1行ずつ足す。**利用者については書かない**（範囲は `docs/requirements.md` 4.9、形と上限は 7.1）。会話の文面はディスクに届かない                                                                                                                                                                                                                                                                                                             |
...
段差として数えられない）。だから足すのは色相ではなく**地の段**にする。

**足すのは導出を1つだけ。差せるつまみは4つのまま、状態の3色も増やさない。** 上の表に1行だけ
加える:
...
（2026-09-21 に実測。やり取りの最後の本文は必ず最終レポートなので、その中のインラインコード・
...
**「Slack のような1行への色付け」に印は足さない。** 実測でいちばん強く読めていたのは
` ```diff ` の行で、**行を塗る記法は既に1つある**（地を塗る色付け。比 1.36:1 に色相が乗る）。
`report-notation.ts` に行を塗る印を足すと `badge` と意味が二重になり、色だけで意味を伝える入口に
なる（13.1 原則5）。代わりに、**意味を持つ塊の地を、既にある色で1割だけ染める**
（`color-mix(in srgb, var(--surface), var(--state-warn) 10%)` の形）。染めてよいのは
`state-ok` / `state-warn` / `state-ng` と `accent` の4つだけで、**新しい色相は1つも足さない**
（1割の染めは地との比 1.19〜1.26:1、その上の `ink` は 10:1 以上を保つ）。

**素の `note`（結論）は引用と見分けが付いていない。** どちらも灰色の 3px の縦罫で、地の差も無い
（実測。`note` の縦罫は `ink-quiet`、引用の縦罫は `rule`）。**いちばん読ませたい印がいちばん弱い**
...
**最終レポートに印を付ける。** 実測では、中間レポートと最終レポートの差は地（1.096:1）・枠
（実線と破線、`rule` で 2.55:1）・小さなラベルの有無の3つで、**地はほぼ見えていない**ので実質
ラベル1つが支えている。最終レポートには次の2つを当てる:

- **ラベル「最終レポート」**。中間レポートのラベルと同じ段・同じ色（`--font-label`・`ink-quiet`）に
  置く。**同じやり取りに中間レポートが1つ以上あるときだけ出す** — 本文が1つしか無いやり取りでは
  「最終」が何も区別せず、内容を持たない行になる
- **地を `surface-raised` に上げる**。中間（沈む）・本文（基準）・最終（浮く）の3段が
  「途中 / ふつう / 結論」の順に並び、**色だけに頼らずに**並べたときの上下が付く

**レポートの見出しと表に、キャラクターの色の線を1本ずつ通す（2026-09-21 に決定を広げた）。**
それまでレポートの中で `accent` を持つのは縦罫の2つ（依頼の見出し・お願い）だけで、**本文を
仕切るもの——見出しと表の見出し行——は誰が来ても同じ灰色**だった。広げるのは**罫線の側だけ**で、
...
- **解き方は、同じ行をもう一度押す**の1つだけ。最新へ戻る専用のボタンは置かない
- **新しいセリフが来ても選択は解けない。** 選ぶこと自体が「読み返している」という意思表示なので、
  下端付近に居るときだけ最新へ寄せるスクロールの規則と同じ立場に置く（読み返しの最中に攫わない）
...
**キャラビューにだけ背景画像を敷ける**（2026-09-21 決定。ユーザーの問い「メイン画面、キャラ画面は
背景をカスタマイズできたほうが楽しいと思ったけどどうかな?」）。**背景はキャラクターパックの
...
- **効かせるのはキャラビューだけ。** メインビュー・サイドバー・入力欄には敷かない。主な仕事は
  長く読み続けることなので（13章）、**読む面の地は誰が来ても同じ**にする（13.1 原則1 の延長。
...
**過去の吹き出しは要素の `opacity` で薄めるのをやめ、色で薄める**。いまの
`.balloon:not(:first-child) { opacity: 0.5 }` は**吹き出しの地ごと半透明にする**ので、背景画像が
文字の裏へ透けて上の2が崩れる。実測では**覆いを 0.9 まで濃くしても 4.27** にしかならず
（画像が真っ白のとき）、`veil` をいくら上げても 4.5 に届かない。**地は不透明な `surface` のまま、
文字の色だけを地へ混ぜて薄める**ことにし、**混ぜる量の上限は 45%**（4.83。50% だと 4.24 で
下回る）。これは**背景を敷かなくても今すでに 4.20 で下限を割っている**ところの手当てでもある。

#### どう敷くか

- **敷く先はキャラビューの領域そのもの**（`.layout-character`）。**枠も角丸も付けない**
...
- 画像の URL は立ち絵と同じ `/character/<file>` で、**取り直しの印（`?v=`）も同じ**（7.1）。
  配る側に足す経路は無い
- **雑談モードでは背景も消える。** キャラビューの領域を畳むので（13.7）、立ち絵と一緒に背景も
  出なくなる。**メインビューへは持ち込まない**（上の「読む面の地は変えない」）
