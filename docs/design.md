...
2026-09-21 決定。**置かない。** 要約を作るのは Claude Code の圧縮（`/compact`。閾値と契機は
`docs/requirements.md` 4.9「記憶の圧縮と忘却」）で、できた要約は**claude 自身の transcript の中に
1行として残る**（`~/.claude/` の下、区切りの直後）。`resume` はそれを読むので、**tsukumo が
`~/.tsukumo/` に写しを持つ必要が無い。**

- **ユーザーは 2026-09-21 に「要約は `~/.tsukumo/` の下に持ってよい」と認めたが、この例外は
  使わない。** 使えば会話に由来する文章の在り処がもう1つ増え（9章）、組み込みの要約は会話の文面を
  そのまま引くことがあるので `docs/coding-standards.md`「会話内容の扱い」の「生の文字列を写さない」と
  擦れる。**写しが無ければ、擦れる場所そのものが無い**
- 写しがあって得られたのは「印を見失ったときに次のセッションへ記憶を渡せる」ことだけで、それは
  `docs/requirements.md` 4.8「復元できなかったときどうするか」が既に諦めている範囲。セッションを
  跨いで残したいキャラクターの属性は**覚えたこと**（上の節）が担う
- **画面に出す区切りにも写しは要らない。** 圧縮はその場では流れてくるメッセージとして、起こし直した
  あとは `getSessionMessages({ includeSystemMessages: true })` が返す区切りとして拾える

**`~/.tsukumo/` に入るものは増えない**（`state.json` とキャラクターパックのディレクトリだけ。9章）。
...
| ディスク                             | 会話は書かない。`bun build` の出力もメモリ。`localStorage` に置くのは領域の比率だけ（キャラクターパックへ書くのは**会話ではなくキャラクターの属性1行**だけ。下の行）                                              |
...
| 雑談の記憶の要約                     | **新規**。作るのは claude 自身の圧縮で、残るのは claude の transcript の中だけ。**tsukumo は写しを持たない**（7章）。tsukumo がするのは容量を数えて圧縮を頼むことと、区切りを画面に出すことだけ                   |
