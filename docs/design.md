...
- **1〜5 は左から詰め、7・8 は右端に寄せる**（`margin-left: auto`）。6 は 5 の右に置き、
  **幅が足りないときに最初に縮む**（要約の末尾が「…」になる）。6 の字が伸び縮みしても 7・8 は
  動かない
- **歯車（設定の画面の口）は今回置かない**（2026-09-23 のユーザーの指定）。設定の画面は作る予定だが
  デザインが未定で、作るときに口の置き場ごと決める。置くなら右端（8 の右）
