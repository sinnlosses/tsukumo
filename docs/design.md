...
（仕事 / 雑談・モデル・許可モード）が帯へ移る。**帯の高さは見本どおり 60px**（2026-09-23 の
ユーザーの選択。見本は `docs/history/mockup/sidebar-tasks-2026-09-23.html` の `<header>`。
30px との比較は下の「帯が奪う面積（実測）」）。
...
- **帯の形**（2026-09-23 決定。値は見本の `<header>` から写し、色はトークンに宛てる）:
  - **画面の端から端まで出す。** 帯は body の padding の外へ負の margin ではみ出す（padding の値は
    `--page-padding` の1箇所）。外寸は**上端の線 3px（`--screen-nav-accent-line`）＋帯の地 60px
...
- **丸く切り抜く**（`border-radius: 50%` と `object-fit: cover`）。透過の絵の縁が帯に溶けない
  ...
  （`box-shadow: 0 0 0 2px var(--accent)`）を掛ける。帯の高さからの式にしないのは、高さが 60px に
...
帯を 30px（body の padding の内側・gap 0.4rem）から 60px（画面の端から端まで・上端の線 3px と
下の罫線 1px を加えた外寸 64px・gap 1rem）に広げる前後を、同じ疑似セッション・同じ窓の大きさ
（高さ 900px）で `scripts/capture-view.ts` と同じ手順（Playwright で DOM の矩形を読む）で直接
測った値（fake driver。`docs/architecture.md`「手で確かめること」。2026-09-23）:

| 幅   | 領域         | 帯 30px（これまで） | 帯 60px（いま） |            差 |
| ---- | ------------ | ------------------: | --------------: | ------------: |
| 1400 | メインビュー |               483.6 |           467.0 | -16.6 (-3.4%) |
| 1400 | キャラビュー |               322.4 |           311.4 | -11.0 (-3.4%) |
| 390  | メインビュー |               482.9 |           482.9 |             0 |
| 390  | キャラビュー |               152.0 |           152.0 |             0 |

広い画面で grid の上端は 52.4px から 80px へ下がる（奪う高さが 27.6px 増える）。内訳は、帯の外寸が
30px から 64px へ 34px 伸びるうち 16px は上の padding の中へはみ出して打ち消し、帯の下の隙間が
6.4px から 16px へ 9.6px 伸びたぶん。上下の行は `fr` で比例配分されるので、**メインビューも
キャラビューも同じ割合（-3.4%）減る**。狭い画面は帯がオーバーレイで、grid の高さの計算
...
伸びる（実測 61px。下線を罫線に重ねる 1px を含む）ので、床を大きく上回る。
