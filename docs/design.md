...
| ## 5. core と adapter          | サーバ側のモジュールと責務。判断（core）と外の世界に触る境界（adapter）。成果の集め方と日記の保存                        |
...
6. **タスクファイルの出入り**（卒業・節目・消えたファイル。2026-09-25 に足す）:
   ...
   - **消えたファイル**: `end` より前の `D`。消したコミットの親の版（`<コミット>^:develop/task/T-xxx.md`）
     を4の `git cat-file --batch` に混ぜて読み、`status: done` のものを、消えた日の終わりの
...
- 覚えの入れ物は**配線（`src/view-delivery.ts`）で1つ作り、`readCommitCalendar` に渡す**
  （モジュールのトップレベルに可変の入れ物を置かない。`docs/coding-standards.md`）。鍵は日付キー、
  値はその日のコミットの数。覚えていない日が範囲に1日でもあれば `git log` を1回起こして範囲
  全体を数え直し、今日以外を覚える。すべて覚えていれば、今日の分だけを `--since=<今日の始まりの
7日前>` で数える
...
- **書き込む先のパックはコマンドが名前（`pack`）で指す**（2026-09-24。見た目の編集のコマンドの
  8つ（`set-portrait` / `clear-portrait` / `set-outfit-accent` / `set-accent` /
  `clear-chat-accent` / `set-profile` / `set-background` / `clear-background`）がどれも必須で持つ）。
