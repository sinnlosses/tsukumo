...
1. **読む面はキャラクターの色を受け取らない。** レポートの本文・サイドバーの文字・入力欄の
   ...
   吹き出し）と、**機械が指し示す場所**（フォーカスの輪・選ばれたタブ・依頼の見出しの縦罫）だけ
...
3. **機械が付けた名前は等幅、人が書いた言葉は本文書体。** タスクのID・ツール名・キー・モデル名・
   ...
   名前だ」という情報を運ぶ
4. **読む面は行長を測る。** 主な仕事が長時間の読書である以上、行長は設計の対象であって
   成り行きに任せない
...
- **本文の行長は 42 全角で切る**。いまは 1400px の窓で約 67 全角あり、目が戻る先を見失う
...
- **使う人**: `ground` / `surface` / `ink` の3つ。受け渡しの経路は決めていない（設定ファイルか
  画面からかは、実際に変えたくなった時点で決める。**つまみの数と意味だけをここで固定する**）
