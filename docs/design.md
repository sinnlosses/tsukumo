...
- **答え待ちが来たら帯の右端に印を出す**（`state.pending` が空でないとき「答え待ち」を
  `state-warn` で。色だけにしない。2026-09-22 に戻る口の横から帯へ移した。13.9）。
  タブのタイトルの印は隠れていても効いている
...
- **並びは上から** パックのラベルと名前（名前は等幅。13.1 原則3）と「新しく作る」 →
  立ち絵の並び → 差し色 → 背景 → 画面の色（戻る口はこの画面の外＝帯にある。13.9）。パックの持ち物が上、利用者の設定が下（上の「並びで表す」）
...
- **立ち絵をつつくと話しかけてくれる**（2026-09-22 決定。ユーザーの指示「立ち絵をつつく形に
  ...
  - **載せたときの合図は3つ**: **ひと揺らしする**・**指の形にする**・**短い案内をポップさせる**。
    **語彙は「会話を遡る」のホバーと同じ「載せると立ち絵が応える」**で、違うのは応え方だけ
    （あちらは表情、こちらは揺れと案内）。**立ち絵に載せても表情は変わらない** — 遡る先を
    持たないので、変える材料が無い
  ...
  - **ターン進行中は押せない**（モードの `<select>` と同じ立場。理由の文面もサーバが断るときと
    同じ定型文を使う）。**ただし `disabled` にはしない** — ブラウザは `disabled` の要素に
    ホバーもフォーカスも通さないので、**なぜ押せないのかを出す場所が無くなる**。`aria-disabled`
    で伝えたうえで、**案内を理由の定型文に差し替える**。**立ち絵そのものは薄めない**（要素の
    `opacity` は地ごと透かす。13.8）ので、押せないことは**応えないこと**で見せる: 指の形に
    ならず、揺れず、案内が理由に替わる。**雑談のときだけ**で、仕事のメインビューには出ない
...
**行に載せると、0.2 秒ほどで立ち絵が先に応える**（2026-09-22 決定。押す前に「この行は立ち絵に
効く」ことが分かるので、行が押せることに気づける）。**ホバーの合図の語彙は「載せると立ち絵が
応える」**で、立ち絵をつつく側（キャラクターから話しかけてもらう）も同じ語彙を使う。

- **離すとすぐ戻る。** 間を置くのは掛かるときだけで、ログの上を通り過ぎただけでは表情が点滅しない
- **遅れは JavaScript のタイマーで作る**（`chat-view.tsx` の `useHoverPreview`）。**CSS の
  `transition-delay` では作れない** — 表情は画像そのものの差し替えなので、遅らせる対象になる
  遷移が無い
- **ホバーと印は争わない。** 表情は「載せている行 → 留めた行 → 最新」の順に決まり、
  **印はホバーでは動かない**（載せるたびに2箇所が明滅する）。いま載せている行は、
  もとからあるホバーの地の変化で分かる
- **立ち絵の `alt` はホバーでも書き換わる**（出ている絵をそのまま説明する）。**支援技術への
  手当ては足さない** — 立ち絵は `aria-live` の中に居ないので alt が変わっても読み上げに
  割り込まず、そもそもホバーはポインタだけの道で、キーボードでは起きない
- **ホバーの無い道（キーボード・タッチ）は今のまま残す。** Tab で行へ辿り着き、Enter / Space で
  遡る（`role="button"` と自前のキーの受け）
...
- **いま出している画面の口は、狭い画面のタブ（`.layout-tab.is-active`）と同じ示し方**にする
  ...
  13.1 原則1）
- **答え待ちの印（`state-warn` の「答え待ち」）は帯の右端へ移す。** これまで各画面の左上の
...
| 出さない     | 理由                                  |
| ------------ | ------------------------------------- |
| 雑談モード   | 画面ごと入れ替わるので見れば分かる    |
| キャラクター | 立ち絵が居る                          |
| セッション   | 同じ部屋の行は時刻しか違わない（4.8） |

- **これまでサイドバーの「セッション情報」にしか無かった**（`<select>` の選択として）。
...
浅葱 / 萌黄 / 山吹 / 茜 / 藍 / 紫苑 / 朱 / 鈍色 / 若草 / 群青 / 香色 / 墨 で、表示は「〜の間」。
...
- **印（セッションの目印）はポート番号のまま**（4.8 / `docs/requirements.md` 4.8「鍵」）。名前は
  ...
  名前を指定する口も作らない
- **出すのは2箇所**——帯の左端と、サイドバーのセッションの行（`session-switch.tsx`）。狭い画面は
  帯の左端が無いので、名前は「≡」を開いた面の先頭に出す（答え待ちの印と同じ畳み方）
- **サイドバーの行は「名前・M/D HH:MM」で、ポート番号は並べない**。名前はポート番号の1対1の
  言い換えなので並べても見分けの助けにならず、**同じ部屋の行を見分けるのは今までどおり時刻**
...
| 上の2つに付いていた答え待ちの印                | 帯の右端             |

**作る画面の「← キャラクターへ戻る」は残す**（帯に出ない画面なので、置き換え先が無い）。

#### 帯が奪う面積（実測）

窓の高さを帯のぶん（40px + gap 0.5rem = 48px）縮めて `scripts/capture-view.ts` で測った値
（fake driver。`docs/architecture.md`「手で確かめること」）:

| 幅   | 領域         |  いま | 帯を置いた後 |             差 |
| ---- | ------------ | ----: | -----------: | -------------: |
| 1400 | メインビュー | 505.4 |        476.6 |  -28.8 (-5.7%) |
| 1400 | キャラビュー | 337.0 |        317.8 |  -19.2 (-5.7%) |
| 390  | メインビュー | 426.9 |        378.9 | -48.0 (-11.2%) |
| 390  | キャラビュー | 152.0 |        152.0 |              0 |

広い画面では上下の行が `fr` で比例配分されるので、**メインビューもキャラビューも 5.7% ずつ**
減る。狭い画面はキャラビューが `9.5rem` 固定なので、**減るぶんは全部メインビューが被る**。
帯の高さを 40px にするのは、狭い画面のタブと同じ「押せる高さ」（2.5rem 前後）に揃えるため。

#### 狭い画面（760px 以下）

**段を増やさず、いまあるタブ帯（高さ 39.1px）の右端に畳む**。上の表の -48px を避けるための
例外で、**広い画面とは見え方が変わる**ことを受け入れる:
...
押すと3つの口が縦に落ちてくる（会話 / キャラクター / トークン消費）。**開いている間だけの
要素**なので、閉じていれば奪う面積はタブ帯の右端の 44px だけで、段は増えない。
...
- **区画の見出しの右に置く**（サイドバーの「タスク一覧 … 一覧を見る」の横展開。トークン消費は
  ...
  サイドバーは3区画のまま（`docs/requirements.md` 4.2）なので、どの区画にも属さない画面が
  出た時点で置き場が尽きる
