...
| ## 7. キャラクターパック       | `character.json` + `persona.md` + 素材。人格の注入と切り替え、書き戻し、雑談の記憶の要約 |
...
**`~/.tsukumo/` に増えるのはこの1つだけ**（`state.json`・キャラクターパックのディレクトリ・
雑談の要約。9章）。
...
| ディスク                             | 会話は書かない（**例外は雑談の要約の写し1つ**。下の行）。`bun build` の出力もメモリ。`localStorage` に置くのは領域の比率だけ（キャラクターパックへ書くのは**会話ではなくキャラクターの属性1行**だけ。下の行）                                                                                                                                                                             |
