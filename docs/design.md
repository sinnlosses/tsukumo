...
| `diary/`          | 日記。`diary` ツールと保存                                                                | `diary-tool`                                                                                                                                             | `diary`                                                                                  |
...
| `session-driver`      | `chat` `diary` `report` `usage-review` `view-server`                                        | core → core（`diary-tool` `report-review` `port-resolution`）・adapter → core（各ツールの判断）・adapter → adapter（`sdk-driver` → `diary`）            |
| `diary`               | `character-pack` `repository`                                                               | adapter → adapter                                                                                                                                       |
...
| `orca` の文字列リテラル                            | `server/adapter/orca-host.ts` だけ                                        | `server/host/adapter/orca-host.ts` だけ                                                                                                                                                                      | 3                     |
...
| `node:child_process` の許可リスト                  | `server/adapter/` の `orca-host.ts` `bundle.ts` `git.ts`                  | `server/host/adapter/orca-host.ts`・`server/repository/adapter/git.ts`・`server/view-server/adapter/bundle.ts`                                                                                               | 3・8                  |
...
成果の画面の振り返りは、**tsukumo がその日の成果を数えて依頼を送り、キャラクターが `diary`
ツールで書いた日記を受け取って、リポジトリごと・日付ごとにホームへ書き足す**（2026-09-25 決定。
...
- **説明文に書くのは「成果の画面から頼まれた振り返りのときだけ呼ぶ」「1ターンに1回」「断られたら
  直して呼び直す」**。日付は欄に持たない（いま書く日は tsukumo が知っている。下の関所）
...
| 条                                                  | 断る理由の要旨                                                     |
| --------------------------------------------------- | ------------------------------------------------------------------ |
| いま書く日が無い（振り返りのターンでない）          | 日記は成果の画面から頼まれた振り返りのときだけ書ける               |
| 同じターンで既に受け付けた                          | 1ターンに1回だけ                                                   |
...
| 段                   | 入る合図                                                                                       |
| -------------------- | ---------------------------------------------------------------------------------------------- |
| この日のタスクを読む | `reflect-achievement` を受けて依頼を送った（`diary-requested`）                                |
| 日記を書く           | `includePartialMessages` の断片で、メインの `diary` の呼び出しの塊が開いた（`diary-drafting`） |
...
- **段のツール（`usage_review_stage` の形）を採らない**のは、振り返りは1回の `diary` の呼び出しで
  済む短いターンで、段ごとにツールを呼ばせると往復がそのまま増えるため。見直しはスキルが実際に
...
- 3段目の合図は、引数の断片を**その塊が閉じるまでつなぎ**、文字列の中かどうかと入れ子の深さを
  ...
  つなぐ入れ物は駆動の世代ごとに1つで、塊が閉じたら捨てる。**運ぶのは段だけ**で、引数の中身は
...
- **`writing` のままターンが終わったら（成功・止めた・失敗のどれでも）`failed`**、セッションが
  終わったら `idle`。`written` と `failed` は次の `diary-requested` まで残る（13.10 の知らせと
  「書けなかった」の材料）。**起こし直すと `idle` から始まる**（`usageReview` と同じ。復元の
  再生に `diary-requested` は出てこない）
- イベントを流すのは: `diary-requested` は session-manager、`diary-drafting` と `diary-stage` は
  駆動のイベントの変換、`diary-written` は窓口（検査と保存を通したときだけ。見直しと同じく
  handler が流す）
