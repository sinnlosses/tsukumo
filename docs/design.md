...
`docs/requirements.md` 4.9「記憶の圧縮と忘却」。画面は 13.7「雑談のときのサイドバー」）。
別のファイルは持たない。
...
**この表に無い固定の色**が2組ある。**コンテキストの内訳の分類**（`context-*`。分類ごとに違う色で、
凡例に分類名が必ず並ぶ）と、**消費の棒**（`usage-bar*`。期間の札の縦棒とモデル別／ツール別の
横棒。分類の色ではなく「その列の最大に対する長さ」を言うだけ）。どちらも**誰が来ても変わらない**
ので、つまみにも導出にもしない（値は `src/browser/styles/theme.css`）。
...
`accent` に戻る（`src/shared/character.ts` の `effectiveAccent`。`docs/design.md` 13.7）。
...
（帯の上端の枠・仕事/雑談のトグルのいまの側・いま出している画面の口・「いまの作業」の札の
...
とおしゃべり中」。`docs/design.md` 13.9 が決めた「札の頭にキャラクターの名前は置かない」の
...
- **帯の形**（2026-09-23 決定。値は見本の `<header>` から写し、色はトークンに宛てる）:
  - **画面の端から端まで出す。** 帯は body の padding の外へ負の margin ではみ出す（padding の値は
    `--page-padding` の1箇所）。外寸は**上端の線 3px（`--screen-nav-accent-line`）＋帯の地 52px
    （`--screen-nav-height`）＋下の罫線 1px（`--screen-nav-rule`）**で、会話の画面の grid も同じ
    変数から高さを引く。帯の下の隙間（`--screen-nav-gap`）は画面の縁と同じ `--page-padding`
  - **帯の地は `--ground` と `--surface` を半々に混ぜた色**（画面の地より一段明るく、領域の地より
    暗い）。上端の線は `--accent`、下の罫線とトグル・口の間の縦の仕切り（高さ 24px・幅 1px）は
    `--rule`。どれも導出なので、歯車から地・字の色を変えても追従する（新しいトークンは足さない）
...
- **丸く切り抜く**（`border-radius: 50%` と `object-fit: cover`）。透過の絵の縁が帯に溶けない
  ように、丸の地を `--surface-accent` で塗る。**大きさは 36px の固定値**で、外側に差し色の 2px の輪
  （`box-shadow: 0 0 0 2px var(--accent)`）を掛ける。帯の高さからの式にしないのは、高さが 52px に
  決まって式で追う理由が無くなり、式のままだと「≡」の面の顔（26px・輪なし）まで帯の高さに
  引きずられるため
...
| 幅   | 領域         | 帯 30px（最初） | 帯 60px（見本どおり） | 帯 52px（いま） |   30px との差 |
| ---- | ------------ | --------------: | --------------------: | --------------: | ------------: |
