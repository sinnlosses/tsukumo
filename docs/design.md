...
- **最低限そろえさせるのは `default` と `working` の2枚。** 片方から両方へ割り当てはしない
  （作業中の絵が別に無いパックが黙ってできるより、2回選ばせるほうがよい）。必須にするのは
  境界のスキーマ（`portraits` を両方 required）で、書き込む側まで欠けた形が届かない。
  **`working` を必須に据えた根拠（自動で切り替える先）は 2026-09-17 に失効している**ので、
  必須から外せるかどうかは別途見直す
- **作った直後は切り替えない。** 増えるのは `<select>` の選択肢だけで、切り替えは選んだときに
  起きる（切り替えは駆動の起こし直し＝画面の初期化なので、作る操作の副作用にしない）
- 差し色は `default` の1色だけを受け取る。衣装ごとの出し分けと `proud` / `flustered` の立ち絵は、
  作ったあと同じ引き出しの編集の口で足す（**作る口は最低限にする**）
...
| キャラクターの立ち絵・差し色                     | **ずっと**（`~/.tsukumo/characters/<name>/`。7.1）           | 「見た目」の引き出し         |
