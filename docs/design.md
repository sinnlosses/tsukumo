...
- **雑談中は上段の高さを 82% に固定する**（`layout.tsx` の `COLLAPSED_CHARACTER_ROW_TOP`）。
  下段に残るのは入力欄だけなので、仕事のときの比率（既定 60%）だと入力欄が画面の4割を占める。
  **使う人が決めた比率は state に残したまま**描くときだけ差し替え、仕事へ戻すとそのまま効く。
  固定される間は**上下の仕切りも出さない**（掴めるのに効かない仕切りになるため）
