...
| トークン         | 役割                                                   | 誰が差すか                                 |
| ---------------- | ------------------------------------------------------ | ------------------------------------------ |
| `surface-raised` | 本文の地の上に**浮く**小さな塊（機械の名前・結論の塊） | 固定（`surface` に `ink` を1割混ぜて導出） |

`surface-accent` と同じく**計算するだけの値**で、新しいつまみではない。混ぜる割合は1割
（`surface` との比 1.31:1、その上の `ink` は 9.77:1）。読む面の地はこれで3段になり、段が
そのまま意味を持つ:
