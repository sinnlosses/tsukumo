...
2. **最近の話題** / 3. **覚えていること**: 区画の枠（`SidebarSection`）で見出しを出し、**いまは
   空のときの案内だけ**。中身の出どころ・件数・「編集」を押したときの形は後から入れる
   （話題は雑談の要約の見出し、覚えていることは `persona.md` の `## 覚えたこと`）。**「編集」は
   消せる行があるときだけ置く**（空のときに押せても何もできない）
