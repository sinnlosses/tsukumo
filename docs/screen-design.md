...
**仕事 / 雑談を切り替えると、立ち絵は下段のキャラビューと上段のメインの間を 0.25 秒かけて
移る**（2026-09-23 決定。ユーザーの報告「切り替えた瞬間に立ち絵がパッと入れ替わって目が
チカチカする」）。実ブラウザで測ると、元の立ち絵は1フレームで消え、移った先では別の位置・
大きさで透明から現れ直し、差し色と背景も同じフレームで入れ替わっていた。初めて出す表情なら
絵を読み終えるまで約 250ms 場所が空き、本物の駆動では続きの履歴が `hello` のあとに届くので、
表情が既定から続きの表情へもう一度飛んでいた。そこで**起こし直しの `hello` は続きの履歴を
流し終えてから配り**（`server/core/session-launch.ts`。表情が変わるのは切り替えの1回だけになる）、
**立ち絵は表情の数だけ先に読んでおき**（`components/portrait.tsx` の `usePortraitPreload`）、
**モードが変わる `hello` の描き直しを View Transition に載せる**（`stores/session.tsx` →
`lib/view-transition.ts`）。2つの立ち絵は同じ名前（`view-transition-name`）で結ばれて位置と
大きさが移り、表情が違えば移る間に溶けて替わる。残り（差し色・背景・領域の組み替え）は
ページごと重ねて溶かす。**表情は切り替え前に揃えない** — 移った先は別のセッションなので、
その最後のセリフの表情が正しい。長さは立ち絵自身の入り（`portrait-fade-in`）と同じにして、
操作してから落ち着くまでを延ばさない。**移っている間だけ、立ち絵は床に沈めたぶんを自分で
切る**（移る絵は領域の `overflow` の外で描かれ、切らないと足元が床の下まで見えて、終わった
瞬間に消える）。**「動きを減らす」では移さず、その場で入れ替える。**
...
|    5 | いまの作業          | 依頼の手順の一覧が開く        | `state.turn` / `state.pending` / `records` | 下の「いまの作業」     |
