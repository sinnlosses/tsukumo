...
| ### 13.10 成果の画面          | `main` に入った成果を1日ぶん見る画面。日の切り替え・数の札・空の日・ボタン    |
...
**`main` に入った成果を1日ぶんずつ見る画面**（2026-09-24 決定。数え方と依頼の文面は
`docs/requirements.md` 4.11、データの受け取り方は `docs/design.md`「成果の集め方と配り方」）。
帯の4つ目の口「成果」から開き、hash は `#achievement`（`Screen` に `achievement` を足す）。
**開いただけではターンを使わない。**

```
 ┌ 成果 ─────────────────────────────────────────────────────────────┐
 │  ‹ 前の日      今日 9月24日（水）      次の日 ›   今日へ           │  ← 日の切り替え
 │                                                                     │
 │  ┌ コミット ─────┐  ┌ 終えたタスク ─┐                              │  ← 数の札
 │  │   42          │  │   5           │                              │
 │  └───────────────┘  └───────────────┘                              │
 │                                                                     │
 │  終えたタスク                                                       │  ← タスクの一覧
 │   T-xxx  帯のいまの作業に背景のタスクを出す                         │
 │   T-yyy  レポートに出たファイルのパスを押すと、Orca のエディタで…   │
 │   …                                                                 │
 │                                                                     │
 │  [ つくもと振り返る ]                                               │  ← 振り返りのボタン
 └─────────────────────────────────────────────────────────────────────┘
...
上から4つ。**どれも読むだけの面**なので、キャラクターの色（`--accent`）はボタンにだけ載せる
（13.1 原則1）。新しい色のトークンは足さない（地・枠・字は `--surface` / `--rule` / `--ink` /
`--ink-quiet` の既存のもの）。

1. **日の切り替え**: 「‹ 前の日」「次の日 ›」の2つの `<button>` と、あいだの日付、右に「今日へ」。
   日付は「今日」「昨日」を頭に添え、その後ろに「9月24日（水）」の形（年は今年でなければ添える）。
   **今日を見ているときは「次の日 ›」と「今日へ」を `aria-disabled` にする**（未来の日は無い）。
   「前の日」は制限しない（リポジトリより前の日は空の日として出る）。**見ている日は hash の
   `?date=YYYY-MM-DD` に持つ**（13.6 の `pack` と同じく、画面の上に乗る付随情報。リロードで同じ日に戻り、
   ブラウザの「戻る」で前に見た日へ戻れる）。`date` が無い・読めないときは今日
2. **数の札が2枚**: 「コミット」と「終えたタスク」。大きい数字（等幅。寸法はトークン消費の画面の札の数と同じ）と項目名だけ
   で、前の日との比較・目標・グラフは置かない（数を競わせる画面にしない）。狭い画面（760px 以下）
   でも2枚は横に並べる（どちらも短い）
3. **終えたタスクの一覧**: 1行に ID（等幅）と `summary`（本文書体。長ければ折り返す。切らない）。
   並びは ID の数の順。押せない（タスク板の表のような詳細は持たない。本文を読みたければ
   サイドバーのタスク一覧から）。**`difficulty` などの欄は出さない**
4. **振り返りのボタン**: 文言は「**<パックの名前>と振り返る**」（パックの `name`。キャラクターの
   名前をコードに書かない。原則4）。形は入力欄の送信ボタン（`dispatch-send`）と同じ
   組み合わせ（地が `--accent`・字が `--ground`）

**コミットの件名は並べない。** 1日に数十〜数百件あり、この画面で読む量ではない（見たければ
ターミナルの `git log`）。**`develop/progress.md` の小節も並べない**（`docs/requirements.md` 4.11
「何を数えるか」）。
...
| 場面                                           | 見え方                                                                                                                               |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| 空の日（コミットも終えたタスクも0）            | 札は **0 のまま出す**（消すと日によって画面の形が変わる）。一覧の場所に「この日に main へ入った成果は無い。」の1行。ボタンは押せない |
| コミットはあり、終えたタスクが0                | 札は両方出す。一覧の場所に「この日に終えたタスクは無い。」。ボタンは押せる（コミットだけでも振り返れる）                             |
| タスクの記録が無いリポジトリ                   | 「終えたタスク」の札の数を「—」にし、札の下に「タスクの記録が無い」。一覧は出さない                                                  |
| `main` が読めない                              | 日の切り替えと札を出さず、「このディレクトリでは成果を数えられない（`main` が読めない）」の1行だけ。ボタンも出さない                 |
| 取りに行って失敗した（応答が落ちた・時間切れ） | 札と一覧の代わりに「成果を取れなかった。」の1行。日の切り替えは使える（別の日を開けば取り直す）                                      |
| 取りに行っている間                             | 前に出していた日の中身を薄くして残す（新しい日の数が届いたら差し替える）。初めて開いたときは札の数を「…」にする                      |
