...
背景、画面のナビの帯）はここが持つ。層・プロトコル・部品・キャラクターパックは `docs/design.md`。
...
**節の番号は `docs/design.md` の13章だったときのまま**（`13.1`〜`13.9`）。コード・テスト・
...
|    4 | 3つの口             | その画面へ移る（`<a href>`）  | `location.hash`                                                | 下の「帯に何を置くか」               |
...
- 出すのは **`Screen` の3つすべて**（会話 `#` / キャラクター `#character` /
  トークン消費 `#token-usage`）。**新しく作るはダイアログ**（キャラクター画面の上に重なる。
...
- **続きの報告**: 背景のタスクが終わると、claude は依頼なしで続きを報告するターンを始める
  （`docs/requirements.md` 4.1）。tsukumo はこれを**記録を持たないターンの始まり
  （`turn-started`）**として扱う。札は「作業中」に戻り、吹き出しは空になってから続きのセリフを
  出し、入力欄の経過時間も測り直す。**メインビューには新しいやり取りを立てず、元の依頼の
  やり取りに積む**（依頼の記録が無いため）。続きの報告がそのやり取りの締めの本文になり、前の
  本文は中間レポートに回るか落ちる（`src/shared/main-view.ts` の選び方のまま）。続きのターンが
  走っている間は、そのやり取りの本文を出さない（まだ伸びうる本文を出さない、と同じ理由）
