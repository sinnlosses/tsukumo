...
- **切り替えは `location.hash`**（`#character` / `#character/new`。無ければ会話の画面。
  キャラクター画面で選んでいるパックも `#character?pack=<名前>` として同じ hash に乗る。下の
  「キャラクター画面の中身」）。
...
- **一覧（左・幅 260px）**: 見出し「キャラクター」と件数、パックごとの行（丸い顔・名前・
  ...
  点線の「新しく作る」（`#character/new` へのリンク）。行はただのリンクで、選んでいる行は
  `aria-current="page"`（差し色で薄く染めた地と太字。色だけにしない）。顔を持たないパックは点線の丸
  （13.9「顔」と同じく、立ち絵や `mini` から切り抜いて代わりにしない）
- **選んでいるパックは hash に持つ**（`#character?pack=<名前>`。2026-09-24 決定）。再読み込み
  しても同じパックが開き、「戻る」で前に選んでいたパックへ戻れる。**`#character/<名前>` にしない**
  のは作る画面の `#character/new` とぶつかるため（`new` はパックの名前に使える）。`pack` が無い・
