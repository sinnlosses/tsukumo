...
「解くべき論点」（`develop/tasks.json` T-502 の本文）への回答:
...
- **例外は日記の本文だけで、端末の明朝体に寄せる**（2026-09-25。成果の画面の吹き出しと見開きの
  右ページ。13.10）。書体の指定は端末に入っている明朝体の名前を並べ、最後を `serif` にする。
  **外から取らず、書体ファイルも同梱しない**（上の「外部フォントは足さない」と同じ理由）。
  明朝体の無い端末では `serif` に落ちるだけで、読めなくはならない。寸法は上の段のまま
  （本文の 15px）で、新しい段は足さない
...
（`features/main-view/reveal/use-report-reveal.ts`）の両方が読むので、機能どうしの import を増やさず
...
- **速いは3つとも標準の半分**。下限・上限を一緒に半分にしないと、短い塊が下限に張り付いた
  ...
  塊の大きさをこの3値で時間に直す）
- **切ると演出そのものを走らせない**。`features/main-view/reveal/use-report-reveal.ts` の `useReportReveal` が
...
- **名乗り（右の最上段）**: 大きな丸い顔（`accent` の輪）・名前・`id: <パックの名前>`（等幅。
  ...
  開くダイアログ（`character-profile-edit-dialog.tsx`）は**作るダイアログ（T-495）と同じ枠・
  同じ欄の書き方**で、欄は「名前」と「ひとことプロフィール」の2つだけ（id は作ったあと変えない
  ので欄を出さない。T-493「決まっていること」）。**ひとことプロフィールは雑談のサイドバーの
...
（2026-09-23 決定。T-335。Discord と同じ、「まだ喋ってくれる」合図）。**セリフの吹き出しの
...
`theme.css` に足し、ここに値を書く（値は見本から取る。作るタスクで決める）。金色の要素には
どれも文字のラベル（「この日のいちばん」「先輩タスクの卒業」「節目」「日記あり」）が必ず添う
（13.1 原則5）。
...
- **書き上がったら**、吹き出しに新しい段落を**書き上げの演出**（レポートの筆が走る演出。
  `features/main-view/reveal/`）で頭から見せ、しおり・暦の鈴を差し替える。演出が掛かるのは
