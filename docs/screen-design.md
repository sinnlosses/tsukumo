...
- **表情は5列の格子**（詳しい設定の幅が 52rem を切ったら入るだけの列で折り返す。`@container`）。
  ...
  `default` のカードには「いつもの顔」の札を添える。見出しの添え書きは枚数と「乗せると差し替え・
  消すが出ます」（変えられないパックでは枚数だけ）
...
**セッション情報の帯にいまのコンテキストの使用量を1行足す**（キャラクター・セッションの2つの
対の下。ユーザー決定）。「使用 81K / 1M」のように**使っている量と窓の大きさ**を出し、その右に
細い棒（使っている割合ぶんだけ塗る）を添える。**押すとトークン消費の画面（`#token-usage`）へ
移る**（帯の口と同じ `<a href>`）。
...
- **取れないとき・まだ届いていないときも行の高さは変わらない**（文字だけを「取得中…」
  「いまのコンテキストは取れていない」に差し替える。押す先はどちらでも `#token-usage` のまま）。
  札の骨組み（`ContextUsageCardSkeleton`）のような灰色の塊は使わない——1行の文字だけなので、
  実物の文字と同じ行の高さで自然に揃う
