...
### 2026-09-24 成果の画面に「つくもと振り返る」ボタンを置いた（T-514）

押すと画面の数から組んだ依頼を1回送って会話の画面へ移り、キャラクターが `speak` で感想を言う。ターン進行中・空の日は `aria-disabled` で理由を添えて送らない。
...
### 2026-09-24 レポートに出たファイルのパスを押すと Orca のエディタで開けるようにした（T-510）

`Host.openFile`（`orca file open`）と `open-file` コマンドを足し、git 管理下の一覧にあるパスだけを開く（門番は `src/server/core/tracked-file.ts`）。ファイルを指さない相対リンクは素の文字にして、ページが遷移しないようにした。

### 2026-09-24 キャラ画面を小さくしたとき、吹き出しも立ち絵に合わせて縮めた（T-530）

吹き出しの文字・余白・列幅を領域の高さ（`cqh`）に連動する `clamp()` にし、上限を従来の値にしたので既定以上の見え方は変わらない。最小でも吹き出しは立ち絵とほぼ同じ高さ（比 1.01）までしか縮まない（文字の下限 11px で止まるため）。

### 2026-09-24 依頼の履歴の一覧で依頼をコピーでき、飛ぶのは別の口にした（T-529）
...
### 2026-09-24 サイドバーのセッション情報に、いまのコンテキスト量を1行で出した（T-517）

「使用 46.0k / 1.00M」と細い棒を出し、押すとトークン消費の画面へ移る。取り直すのはターンが終わるたびで、そのために状態へ `lastTurnFinishedAt`（次のターンが始まっても戻らない）を足した。

### 2026-09-24 締めのセリフの固定の例を同梱パックから消し、ホームのパックの古い締めの節も揃えた（T-506）

`characters/tsukumo` と `characters/tsukumo-spirit` の「締めのセリフの言い方」から例3項目を消し、お願いの中身を言わない理由を地の文に残した。ホームのパックの同じ節も同じ文面に置き換え、「予告」「note-favor」の古い条が消えた。

### 2026-09-24 API の再試行・失敗・利用上限を受け取り、画面に理由を出すようにした（T-446）

再試行中・利用上限・失敗の理由を入力欄の経過時間の行に1つ出し、失敗で終わったターンはメインビューの末尾に「失敗で終わった」と理由を字で出す。`turn-finished` は `outcome`（completed / interrupted / failed）に替えた。自由文のエラーは運ばない。

### 2026-09-24 見直しの結果を提案の札に並べ、頼む・タスクにする・見送るを押せるようにした（T-502）

結果と「前回の提案」を同じ札の形で出し、主ボタンで依頼を送っても画面には留まる。見送ると札はその場で消え、全部消えたら「いま出せる提案は無い。」を出す。

### 2026-09-24 トークン消費の画面に減らし方の提案の区画を足した（T-501）

ふだんは「減らし方を見てもらう」ボタン、見直し中は5段の進み・経過・直近のセリフ・止めるを出し、題の帯に「見直し中」を置く。雑談中とターン進行中は理由つきで押せない。「前回の提案」のリンクは T-502 が中身を足す。
...
### 2026-09-24 見直しの結果をホームに残し、前回の提案と見送りを配るようにした（T-500）

前回の結果（直前の1回だけ）と見送った提案の識別子を別のファイルに残し、見送りの一覧を `usage_review_stage` の戻り値でスキルへ渡す。取り消しの口は作らず、ファイルから手で消す。起動し直したときは前回の結果から見送り済みを除いて配る。

### 2026-09-24 見直しの結果を構造で受け取るツールと、見直しの状態を配る口を作った（T-499）

`usage_review_stage`（段の進み）と `usage_review_result`（結果を1回で）を仕事モードに載せ、`SessionState.usageReview` を ふだん / 見直し中 / 結果 で配る。見送りの読み手は `() => []` の仮置きで T-500 が差し替える。仕事モードの `report` の関所と見直しのターンの兼ね合いは T-503 で決める。
