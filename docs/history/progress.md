...
### 2026-09-24 用語集に最終レポートを立て、report ツールの設計判断を書いた（T-477）

`docs/glossary.md` に「最終レポート」を足し、`docs/architecture.md` に「レポートはテキストではなく `report` ツールで受け取る」を採らなかった推測方式と並べて書いた。

### 2026-09-24 report が無いターンの受け皿を最後の本文だけにした（T-476）

`report` が呼ばれなかったターンは最後の本文（空白だけは除く）をそのまま最終レポートにし、英訳・実況・繰り上げの判定と `japanese-prose.ts` を消した。中間レポートは `report` からしか生まれない。

### 2026-09-24 「名前とプロフィールを変える」のダイアログを足した（T-498）

詳しい設定の名乗りに鉛筆の口を置き、作るダイアログと同じ枠で名前とひとことだけを受け取って `set-profile` を送る。id は変えられないので欄を出さない。

### 2026-09-24 キャラクターを消す帯と、id を打って確かめるダイアログを足した（T-497）

詳しい設定の最下部に赤い帯を置き、id を完全に打つまで「消す」を押せないダイアログから `delete-character` を送る。同梱を覆うホームのコピーは「同梱に戻す」の文言で出す。

### 2026-09-24 表情を消す前に、カードに添う確かめの吹き出しを出すようにした（T-496）

消す口を押すとカードの右下に確かめの吹き出しが開き、そこでの「消す」で初めて `clear-portrait` を送る。表情の名前はパックのラベルを使う。

### 2026-09-24 新しく作るをキャラクター画面に重なるダイアログにした（T-495）

`#character/new` の画面を撤去し、名前・id・仕事と雑談の差し色・いつもの顔を受け取るダイアログにした。作れたら閉じて一覧でそのパックを選ぶ（自動では切り替えない）。

### 2026-09-24 キャラクター画面を左の一覧と右の詳しい設定の2段組にした（T-494）

選んでいるパックは `#character?pack=<名前>` に持たせ、使用中以外も詳しい設定で直せるようにした。立ち絵の無い表情は名前つきの空欄で出し、差し替え・消すのアイコンは乗せたときとフォーカスで出す。変えられないパックの表示は実物では未目視。
...
### 2026-09-24 受理済みの report を同じ引数で送り直して関所を抜ける穴を塞いだ（T-505）

検査（`report-review.ts`）が、同じターンで描いた `report` と同じ引数の呼び出しを規約違反とは別枠で1ターン1回まで差し戻し、関所の理由文にも送り直さないよう1文足した。関所が止めたあとの続きで `session-info` が届き直さないことは前提にしていて、実セッションでは未確認。
...
### 2026-09-24 編集コマンドに対象のパック名を持たせ、使用中以外も直せるようにした（T-491）

見た目の編集コマンドは `pack` を必須で持ち、書き込み先は一覧と突き合わせて引く（名前はパスに使わない）。使用中以外を変えても持ち替えず・起こし直さず、一覧だけ配り直す。断る理由は既存の `characterEditFailed` を流用した。

### 2026-09-24 docs を引く「句」が参照先に実在するかを数える道具を作った（T-484）

`scripts/section-reference.ts` と `test/section-reference.test.ts` で「ファイル名＋番号＋「句」」の参照を拾って照らし、`bun run check` で0件を保つ。迷子25件を直し（向け直し19件・参照を外し6件）、`docs/research/` は調査時点の記録として拾う対象から外した。

### 2026-09-23 トークン消費の記録から削る先を提案するスキルを作った（T-377）

`~/.claude/skills/token-usage-diet/` にスキルと集計スクリプトを置き、`token-usage/` と `context-usage/` を畳んだ表（93行）だけから「何を・どれだけ減らせそうか・根拠の数」を出させる形にした。MCP ツールだけは呼び出し実績と突き合わせて未使用を断定でき、スキルは呼び出し名が記録に残らないため重さの一覧に留める（`speak`・`report` は tools[] に残らないので突き合わせから除外）。tsukumo 本体（`src/`）には手を入れていない。
...
### 2026-09-23 T-447（ローカルコマンドの出力をメインビューに出す）を着手しない判断で閉じた

本文の前提「出力が画面に出ない」は実物と違っていた。`/usage` の出力はメインビューに出ていて、残る不便は改行が潰れてレポートと見分けがつかないことだけだった。ユーザーの判断で、課題感が生まれるまで見送る。`status` を `done`・`passes` を `false` のままにし、`evidence` に理由を書いた。実装も検証もしていない。
...
### 2026-09-23 雑談の索引で、同じ日の見出しを全部照合するようにした（T-480）

`readDayIndexHeadings` が同じ日の行を「あとの行が勝つ」で1行に畳んでいたので、1日に何度も `index` を書くと前の行の語で `recall` が当たらなかった。日ごとに全行を持ってどれか1行に当たればその日を拾う形にし、docs 4ファイルから「1日1行」の記述を消した。

### 2026-09-23 T-449（帯に Claude Code の Todo をチェックリストで出す）を着手しない判断で閉じた

会話で「これをすることの嬉しさは何か」と問われて評価した結果、ユーザーの判断で実施しないと決めた。帯（画面最上部のナビ）に n / N を出すのは Claude Code の TUI の焼き直しで、効くのは長い依頼で席を立つときだけ。進み具合をセリフで言うのに tsukumo 側で Todo を解析する必要はない——`speak` を呼ぶ claude 自身が Todo を持っているので `src/server/core/speech-cadence.ts` に書けば済む。帯の枠は T-448（背景で動くタスクを帯に出す）に使う。`status` を `done`・`passes` を `false` のままにし、`evidence` に理由を書いた。実装も検証もしていない。
...
### 2026-09-23 レポートの記法の印の名前を shared の1ファイルに置いた（T-437）

印の名前とラベルを `src/shared/report-notation.ts` に集め、画面の対応表はそこから引く。claude に教える文面は1字も変えず、語彙の各印が文面・部品・CSS まで届くことをテストで見張る形にした。
...
### 2026-09-23 bun run grid を常駐させ、読み込むたびに部屋を走査し直すようにした（T-482）

格子は一時ファイルではなく `node:http`（空きポート・`/room-grid?t=鍵`・`no-store`）で配り、ブラウザの再読み込みでもページ内のボタンでも並べ直す。格子のタブが消えるとプロセスも終わる。打ち直しで前のタブを閉じる動きは、他セッションの格子タブを巻き込むので実機では見ていない。
...
### 2026-09-23 仕事と雑談を切り替えたときの立ち絵のチカチカを直した（T-478）

切り替えの1フレームで立ち絵が消えて別の領域に現れ直し、初めての表情は約250ms 空き、本物の駆動では履歴が `hello` の後に届いて表情がもう一度飛んでいた。起こし直しの `hello` を履歴の再生後に配り、立ち絵を先読みし、モードが変わる `hello` を View Transition に載せて0.25秒で移すようにした（`docs/screen-design.md` 13.7）。Orca のタブと本物の SDK での切り替えはまだ目視していない。
...
### 2026-09-23 雑談中のサイドバーをプロフィールの札・話題・覚えていること・セッションの4段にした（T-332）

`<Sidebar>` が `chatMode` を見て自分で差し替える（`<Root>` は分岐しない）。部品は `features/sidebar/` の中に置き、「変える ⌄」は見た目のボタンに透明な本物の `<select>` を重ねた（帯の `<select>` と同じ `character-switch.tsx`）。`character.json` に任意の `tagline`（ひとことプロフィール）を足し、同梱の2パックに書いた。見本の橙はパックの `--accent` に写し、字と枠は `ink` / `ink-quiet` / `rule`。論点と採らなかった案は `docs/design.md` 13.7「雑談のときのサイドバー」。最近の話題（T-390）と覚えていること（T-391）は見出しと空の案内だけで、「編集」は消せる行ができたら置く。
...
### 2026-09-23 帯を 60px にし、上端の線・地と罫線・文字の口・顔と部屋の名前を見本に揃えた（T-384）

帯は負の margin で body の padding（新設の `--page-padding`）の外へ出し、上端の線 3px と罫線 1px は `--screen-nav-height`（60px）とは別の変数にした。顔は 36px の固定値、帯の下の隙間は見本どおり 16px。`docs/design.md` 13.9 の面積の表を 30px / 60px の比較に書き直した。
...
### 2026-09-23 サイドバーの下端を顔つきのキャラクターとセッションの選択にした（T-389）

セッション情報を区画から外し、サイドバーの左右いっぱいに広がる下端の帯（`.sidebar-footer`。
上端の罫線と一段沈んだ地）にして、キャラクターとセッションの2つの `<select>` を小さなラベル付きで
横に等分で並べた。顔は帯とサイドバーの両方が読むので
`src/browser/components/character-face.tsx` へ上げて共有した。**寸法は見本
（`docs/history/mockup/sidebar-tasks-2026-09-23.html`）から取り直した**——顔 28px・ラベル
`--font-label`・`<select>` 高さ 34px。狭い画面でも1列 154px 取れるので縦積みは入れていない。

### 2026-09-23 メインビューの札の頭を、字を詰めて潰れない範囲で低くした（T-420）

前後の口を 32px から 24px（13.9 の床）に、タイトルを 18px から 14px の「区画の見出し」の段に落とし、タイトルと右端の行の高さを詰めた。18px の段は他の画面が使うので残し、13.3 の表の行名を「大きな見出し（画面・ダイアログの題）」に直した。

### 2026-09-23 画像の札に max-width を足し、横長の絵で × ごと切れるのを直した（T-415）

横長 800x300・縦長 300x800 では札と絵の箱は一致し × は角から 2.39px で、報告された内側へのずれは再現しなかった。実測で見つかった欠陥は `.prompt-image` の `max-width: 100%` が循環して効かず、1400x100 などで × ごと画面外に切れることで、`<li>` 側に `max-width: 100%` を足して直した。
