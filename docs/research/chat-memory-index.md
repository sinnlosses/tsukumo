...
## `/compact` をやめたあと: 起こし直しで文脈を切る

**`/compact` と `PostCompact` の写しはやめる。** 代わりに、`resume` で続けているセッションの文脈を
縮める役は**起こし直し**が受ける。

- 起こすとき、前に起こしてからアーカイブに積まれた逐語が `recentBytes` を超えていたら、`resume`
  せずに新しく起こす（載るのは作業記憶・あらすじ・覚えたこと）
- 超えていなければ今までどおり `resume` する

**未決**: 新しく起こすと、画面のログ（transcript からの復元、`docs/chat-mode.md` 4.8）が空になる。
アーカイブから直近を画面に復元する口を作るか、空で始めるのを受け入れるかは決めていない。
...
- 画面のログをアーカイブから復元するか（上の「`/compact` をやめたあと」）
