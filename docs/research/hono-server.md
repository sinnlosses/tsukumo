...
2. **本当の利得は行数ではなく、T-123 で「パーサーが無いから」と退けた経路（multipart の POST）が
   使えるようになることと、トークン・`Origin` の照合が全経路で1つのミドルウェアになること**
3. **今は入れない。** 入れ時は「経路が5つを超える」「ファイルを HTTP で受けると決める」「T-046 で
...
2. **トークンと `Origin` の照合が1つのミドルウェアになる。** T-123 の却下理由「生バイト POST は
   トークンと `Origin` の照合をもう一組書く」が消える。**経路を足すたびに守りを書き直さなくてよくなる**
3. **multipart が使える。** T-123 の却下理由「`node:http` にパーサーが無い」が消える。画像を data URL で
...
5. **Node / Bun / Electron で同じコード。** T-046 で Electron の Node へ寄せるとき、`node:http` 直書きより
...
| ファイルを HTTP で受けると決める                   | T-123 で WebSocket の data URL に決定済み（T-124/125 で実装） |
| T-046 で Node（Electron）へ寄せると決める          | 未決                                                          |
