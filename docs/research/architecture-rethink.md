...
1. **描画がサーバ側の文字列組み立てに寄っている。** `presentation/view.ts` 1,628 行 + そのテスト 3,224 行。
   テストは HTML の文字列一致なので、見た目を1箇所変えると大量に落ちる（画面を割り直す作業が「1バイトも変えない」を
   完了条件にせざるを得ないのはこのため）。
...
4. **Markdown レンダラとサニタイザが自前。** 引用・ネスト・水平線・列揃えが描けず、
...
**2026-09-12 の「フレームワークを入れない」判断は当時の3つの不足に対しては正しかった。**
...
- **ブラウザは同じ `applySessionEvent` で畳む**（isomorphic）。サーバとブラウザの状態が構造的に一致し、
  セッション復元は「transcript から起こしたイベントを流す」だけで両側とも戻る
...
| Markdown       | **unified**: `react-markdown` + `remark-gfm` + `rehype-raw` + `rehype-sanitize`   | `streamdown`（Vercel。流れる Markdown 専用） | 引用・ネスト・水平線・列揃えが最初から描ける。`rehype-sanitize` の schema に `note` / `badge` / `cols` / `card` の語彙を許可すれば自前サニタイザ（398行）が消える |
...
キャラクターの切り替えは、**人格を素材と同じ場所へ持ってくる**のがいちばん筋がよい。
...
- `docs/requirements.md` 2.2「外部の Markdown ライブラリを同梱しない」: **覆す**。
  理由は上の症状4
...
| 段  | やること                                                                                                                                         | 効き目                                                                                                              | 併走 |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------- | ---- |
| 0   | **Markdown を unified に置き換える**（サーバ側で `remark-parse → remark-gfm → remark-rehype → rehype-raw → rehype-sanitize → rehype-stringify`） | 描けない記法の不足が解消し、`report-notation.ts` から「描けない記法」の迂回が消える。**今の形のままでも価値がある** | 可   |
...
| 5   | キャラクターパック（`persona.md`）と切り替え                                                                                                     | 人格と素材が1単位に                                                                                                 | —    |
| 6   | `SessionManager` を鍵付きにし、必要なら client–daemon へ                                                                                         | 複数プロジェクト・ポートずらしの廃止                                                                                | —    |
| 7   | 箱（Electron or Tauri）                                                                                                                          | `loadURL` するだけ                                                                                                  | —    |

段0 と段1 は今進んでいる作業とぶつからない。**段2 に入るなら `view.ts` を
割る作業は着手しないほうがよい**（割った先ごと消える）。
