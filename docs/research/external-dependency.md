...
| **同梱物を `import.meta.url` から解くこと**                                   | `bundledFilePath`（`src/core/bundled-path.ts`）が `new URL("../..", import.meta.url)` を基準にする。見ているのは `vendor/`・`characters/`・`src/ui/`・`test/fixture/fake-session.json`（`src/core/fake-driver.ts` の `DEFAULT_SCRIPT_URL`）                                                                               | ソースツリーごと置かれていないと同梱物が読めない（立ち絵なし・`vendor` が404・偽の駆動が起動しない）                                                          | **要検討**   |
