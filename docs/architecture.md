...
- **原則5**: 1ファイルにまとめるか分けるかは、行数でも関数の数でもなく
  ...
  **置き場所を名前にしたファイルは作らない**。**ファイルは単数形**にし、複数は「複数返す」
  関数名の側で表す（`readTaskSummaries`）。**ディレクトリも単数形。ただし置き場所を名前にした
  ディレクトリ（`src/browser/` の `features/` `components/` `stores/` `styles/` と、どの層にも
  作ってよい `lib/` `utils/`）だけは bullet-proof-react の名前をそのまま使う**
  （2026-09-16 と 2026-09-21。箱ごとに何を置くか・`lib/` と `utils/` のどちらに置くかは
  `docs/design.md` 2章「`lib/` と `utils/` に置く基準」が正典）
...
#### worktree でセッションを分ける（2026-09-22）

**複数の tsukumo を1つの作業ツリーで動かすのをやめ、セッションごとに git worktree を切る。**
...
worktree.ts（adapter）」）。**1タスクごとのマージ（下の3）も入っている**（`mergeWorkspace`。
呼び出し元の配線だけが別のタスク）。

**1. 1つ目のセッションも切る。本体で作業するセッションを作らない。**
「最初の1つは本体、2つ目から worktree」にすると、**自分が何番目かを起動のたびに判断する**ことに
なり、その判断にはセッションをまたいだ観測（誰が本体に居るか）が要るので、いちばん壊れやすい場所に
競合を1つ足すことになる。全部切れば分岐が消え、**本体の作業ツリーは誰も書かない場所**に固定できる
——これは代償ではなく利点で、`git -C <本体> merge` が「汚れていない相手へのマージ」で済み
（本体が汚れている場合は移行期の例外に落ちる）、ユーザーが本体で普通に git を使うのも自由になる。
切る費用は `git worktree add` 0.14秒 ＋ `bun run build` 0.05秒で、1タスクごとに畳むので溜まらない。

- **例外は2つだけ**（「場合による」を増やさない）。**git リポジトリでないディレクトリで起こした
  とき**は切らずにそのまま動く（分けるものが無い）。**環境変数で明示的にやめたとき**
  （`TSUKUMO_WORKTREE=0`。`TSUKUMO_OPEN_VIEW=0` と同じ「`0` のときだけやめる」の書き方）は
  本体で動く
...
**worktree で直した自分自身のコードが効くのは「本体へマージする → 起こし直す」の後**で、
tsukumo は自動では起こし直さない（配信中のビュー・メモリ上の会話・繋ぎっぱなしの WebSocket を
持ったまま足元のコードを入れ替えると、どの版が動いているのか誰にも分からなくなる）。

- **マージ前に絵を見たいときは、worktree の中でポートをずらしてもう1つ起こす**
  （`TSUKUMO_VIEW_PORT` を変えて `bun run dev`）。そのプロセスは worktree のコードで動く。
  **本体で動いているプロセスの見張りに worktree を見せない**——`bun run dev` の見張りは自分が
  起きたディレクトリの `src/browser/` だけを見る、といういまの形を変えない
- **`dist/browser/` は `.gitignore` なので、マージで成果物が入れ替わることはない。** マージの
  あと本体の成果物が古くなることはあるが、**起動時に古さを1行で知らせる仕組みが既にある**
  （上の「ブラウザ側は事前に組み立てて置く」）ので、そこに任せて自動では組み立て直さない
- **「自分のリポジトリかどうか」の判定をコードに持ち込まない。** これは tsukumo が自分自身を
  開発するリポジトリであることから来る問題だが、**運用（マージ → 起こし直し）と表示で解く**。
  分岐を持つと、他のプロジェクトで起こしたときに効かない道が1本増える
- **画面には、プロセスのコードの出所（本体）と claude の作業先（worktree）を両方出す。**
  この2つが食い違っているのが常態なので、読み取れないとデバッグのたびに迷う

**3. マージが衝突したら、本体を `merge --abort` で戻し、worktree は畳まずにそこで止める。**
自動で解こうとしない（`-X ours` / `-X theirs` も使わない）。順序は「**まず本体を元に戻す** →
**成果を残す** → **何が起きたかを画面に出す** → **そのセッションは次のタスクへ進まない**」。
半端なマージ状態の本体を放置すると、次に起きたセッションがそこへマージしに来て被害が増える。
worktree とブランチを消さないのは、**解くのに要る材料がそこにしか無い**ため。

- 画面に出すのは、**ブランチ名・worktree の絶対パス・衝突したファイルの一覧・次の手**
  （worktree 側で `git merge main` して解き、もう一度マージを頼む）。会話の内容は混ぜない
- 出し方は**会話の記録（`SessionState` の `records`）に混ぜず、状態の別の場所**に持つ。
  claude の発言ではなく tsukumo が出した知らせなので、ターンの流れに並べない
- **マージの形は「fast-forward できるならそれ、できないときだけ merge commit」**（`git merge`
  に `--ff-only` も `--no-ff` も付けない。2026-09-22 実装時に決定）。`--no-ff` を常に付けると
  中身の無いコミットが1タスクごとに積まれ、`--ff-only` だと**他のセッションが先にマージした
  だけで止まる**（衝突していないのに人を呼ぶ）
- **本体に未コミットの変更があるときはマージを始めない**（同日決定）。重ならないファイルなら
  `git merge` は通るが、そのあと衝突して `merge --abort` したときに**マージ前からあった
  未コミットの変更を git が復元しきれないことがある**。本体は誰も書かない場所という前提
  （上の1）が崩れている状態なので、黙って進めずに上と同じ形で画面に出して止まる
- **畳むのは「その worktree を使っているセッションが生きていないとき」だけ。** 自分の worktree は
  マージしても畳まない（`cwd` が消えると claude の立っている場所が無くなる）ので、**プロセスが
  終わったあと、次の起動の掃除が畳む**（下の「後片付けが漏れない形」と同じ口）。畳めなかった
  ときは**印を残す**ので、次の起動がもう一度見つける
- **知らせは `SessionState` の `workspaceNotices`**（会話の記録には混ぜない）。サイドバーの
  「セッション情報」の先頭に改行のまま出す。**作業先・ブランチ・コードの出所の行は無くなった**
  （2026-09-22 T-365。パスは部屋の名前の `title` に移った——`docs/design.md` 13.9
  「部屋の名前」）

**4. 置き場は `.git` の下、名前は切った時刻。**
`git rev-parse --git-common-dir` が返す場所の下に `tsukumo/worktree/<名前>` を作る
（例: `<本体>/.git/tsukumo/worktree/20260922-153012`）。**排他の印と同じ親の下**に置くので、
**git に1回聞けば worktree も印も見つかる**（置き場を別に決めて回らない）。ブランチ名は
`tsukumo/<名前>` で、`<名前>` は切った時刻（`YYYYMMDD-HHMMSS`。同じ秒に2つ起きたら `-2`,
`-3` と足し、**ディレクトリの作成が成功したほうが勝つ**）。

**タスクid をブランチ名に入れない**のは、切るのが起動時でタスクを取る前だから、そして
**タスク運用の無いリポジトリでも同じ形で動く**ようにするため。

- 2026-09-22 に `/tmp` の使い捨てリポジトリで確かめたこと: 切る → worktree でコミット →
  本体のディレクトリを指してマージ → `git worktree remove` → `git branch -d` が一周通り、
  本体の `git status` は最後まで clean で、取り残しのディレクトリも出ない。
  **`bun test` は `.git` の下を走査しない**（本体で走らせても worktree 側のテストを拾わない）。
  同日、実装した `mergeWorkspace` で同じ一周（切る → コミット → マージ → 畳む）と、
  **衝突・本体が汚れているときに止まること**も同じ形の clone で確かめた（fast-forward で
  本体が進み、`git worktree list` から消え、`tsukumo/*` のブランチも残らない）
...
| 案                              | 採らない理由                                                                                                      |
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------- |
| `/tmp` の下                     | OS が掃除する・再起動で消える。**未マージの成果が黙って消える**                                                   |
| リポジトリの隣                  | ghq の名前空間（`~/ghq/<host>/<user>/`）を汚す。リポジトリを消しても worktree が残る                              |
| リポジトリの中（`.worktrees/`） | `.gitignore` が要るうえ、ツールの走査と見張りが自分の複製を拾う                                                   |
| orca の workspaces の下         | **置き場を orca に決めさせると、orca に聞かないと自分の worktree が分からない**（メタデータは表示にだけ使う原則） |
| `~/.tsukumo/` の下              | ホームは `TSUKUMO_HOME` で丸ごと移せる。移した瞬間に worktree の居場所が変わり、**リポジトリと切り離される**      |

- **後片付けが漏れない形**: worktree ごとに「使用中」の印（pid つき）を印と同じ親の下に置き、
  **起動のたびに全部見る**（**印と実体の両方から集める**ので、印を人が消しても実体だけが
  残り続けない）。pid が生きていないものは、**ブランチに未マージのコミットも未コミットの
  変更も無ければ畳む**（`git worktree remove` ＋ `git branch -d`）。残っていれば**消さずに1行
  知らせる**——取り残しを溜めないことと、成果を黙って消さないことの折り合いをここで取る
- **着手の印は `tsukumo/claim/<タスクid>` に、ファイルの `wx` 作成で取る**（2026-09-22 実装）。
  同時に取りに来ても**作れた側だけが勝つ**ので、取れた／取れなかったを別の仕組みで決めない。
  中身はタスクid・pid・取った時刻・作業先の4つで、**会話の内容は書かない**
- **pid の使い回しは見分けない**（起動時刻と併せない）。他のプロセスの起動時刻を読むには `ps` の
  ような外部コマンドが要り、**依存を増やすには承認が要る**。取り違えても「生きている扱い＝
  取れない」へ倒れるので二重着手にはならず、取り残しは人が印を消せる
- **落ちたセッションの印を掃除するのは、次に取りに来たセッション**（worktree の掃除は起動時、
  着手の印の掃除は取りに行く直前。どちらも pid の生死で決める）。**読めない印も掃除する側へ
  倒す**——読めないものを残すと、そのタスクを誰も取れないまま止まる

**5. 印を取るのも返すのも、tsukumo が出す MCP ツール（`claim` / `finish`）越しにやる**（同日）。
`/next-task` の手順4（`doing` に書き換えるのと同じ時点）で `claim`、手順7（`done` にして
コミットしたあと）で `finish` を呼ぶ。`finish` は**印を返すのと本体へのマージを1回で**やる
——同じ時点で起きることを2つの口に分けると、片方だけ呼ばれた状態が実在してしまう。

- **印の持ち主は tsukumo のプロセスでなければならない**（生死が pid で決まるため）。ツールの
  handler は**プロセス内の MCP サーバ**で動くので、`process.pid` がそのまま常駐している
  セッションの pid になる
- 採らなかった案: **`scripts/` に口を作って `bun run` で叩く**——短命なコマンドの pid が印に
  載るので、**書いた直後に「落ちたセッションの取り残し」に見えて掃除される**（印がまったく
  効かない）。親をたどって tsukumo の pid を拾うには `ps` が要り、依存を増やすには承認が要る
- 採らなかった案: **起動時に `.git` のパスを持ち回ってサーバ側が取る**——サーバは「いまどの
  タスクを進めているか」を知らない（知るには会話の中身を読むことになる）。`.git` の絶対パスを
  `prepareWorkspace` の戻り値へ載せる部分だけは、この案から採ってツールの handler が使う
- **マージが止まった知らせは、戻り値と画面（`workspaceNotices`）の両方へ同じ文面で出す**
  ——人と claude が別々のことを知っている状態を作らない。ツールの戻り値だけにすると、
  claude が本文へ書き写す形になり「会話の記録には混ぜない」（決定3）を破る
...
当たらない（`docs/requirements.md` 4.9「古い雑談は索引を引いて思い出す」に理由が3つ）。

**2026-09-22 に、仕事の `claim` / `finish` も外に置いた**（下の「worktree でセッションを
分ける」の決定5）。返すのは**印のファイルと git の返事**——どちらもそのセッションが `cat` と
`git` で自分で読める外の事実で、tsukumo の状態も画面の事情も載らない。**取れなかった／止まった
ことを返さないと成り立たないツール**なので、"ok" だけにする形は採れない。**広げたのはこの2組
だけ。**
...
空になる側は 2026-09-16 にプレースホルダーを出す形で解いた。`docs/requirements.md` 4.2）。
...
技術に依存しないようにする）。**2つ目のアダプタを書くまで、ポートは `showView` 1つに保つ。**
ホストに頼む仕事を増やさないことが、その日のコストを小さく保つ唯一の手段だから。
...
- **ホストのポートに残っているのは `showView` 1つだけ**（2026-09-12）。ペインの分割・文字送信・
  キー送信は撤去した。ここに操作を足す前に、ページ側で完結しない理由があるかを確かめる
