...
- マージの形（fast-forward だけ許すか merge commit を作るか）と、本体が汚れていたときの扱いは
  ここでは決めない（実装のタスク側の論点）。**`--abort` で戻すことはどちらの形でも変わらない**
...
- **後片付けが漏れない形**: worktree ごとに「使用中」の印（pid つき）を印と同じ親の下に置き、
  **起動のたびに全部見る**。pid が生きていないものは、**ブランチに未マージのコミットも未コミットの
