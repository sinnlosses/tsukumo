...
| `src/browser/app.tsx`                                                        | browser    | `<App>`。Provider の重なりと、出す画面を選ぶ `<Root>`                                                                                                               |
...
（`domain/reveal/use-report-reveal.ts`）が二度と起動しない**。ミニ立ち絵も出ない。実測で、20ターンを超えた
プロセスでは演出が1回も走らなかった。
...
**2026-09-25 に、関所の差し戻しの理由をそのターンで `report` が済んでいるかで分けた。** 済んだ
ターンで「`report` → 締めの `speak` → レポートと同じ中身のまとめの本文」と書いて止まったとき、
一律の理由（「画面に出ていない、`report` で渡し直せ」）に従って中身のほぼ同じ `report` が出し
直され、最初の1枚が中間レポートになって似た2枚が並んだ。締めのセリフも「今度は画面に出した」と
利用者には見えない差し戻しに触れた。済んだターンでは「レポートはもう画面に出ている。言い直しなら
何も呼ばずに終える」と返し、両方の理由に「差し戻しにはセリフでもレポートでも触れない」を足した。
...
（`docs/chat-mode.md` 4.9「雑談モードで変わるもの」）。往復の数は条でも変数でも同じ
（どちらも最後のツール結果への応答1回は要る）。
...
（当時の `docs/coding-standards.md` の節「描画は自動テストで守らない」）、個別ページの経路は使われていなかった。
...
**この決定が縛らないもの**: レポートの表現力（別の領分）と、Markdown に記法を足すこと自体。
...
**ファイルは単数形にする。** 複数は「複数返す」関数名の側で表す
（`unfinishedTaskIds`）。ディレクトリの側の縛りは 2026-09-26 に外した（上の原則5）。
この規則に合っていなかった2つは改名する:
...
3. `presentation` の中の割り方を決める。**残りのブラウザ側 JS を `.ts` へ移すことと、CSS を
   `.css` へ割ることの後**に回す。`view.ts` 3277行から1000行以上が外へ出てから割らないと、割る線が
   二度動く
