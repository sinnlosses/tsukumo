...
- **ブランチは自分で切らない。** コミット先は tsukumo が起動時に切った worktree の
  `tsukumo/<時刻>` ブランチで、本体へは `finish` が1タスクごとに入れる（`CLAUDE.md`「Git運用」と
  「## タスク運用」。着手の印を取る `claim` も同じ節）
- **触る層が重なるタスクでも並行させてよい**（セッションごとに作業ツリーが分かれている）。
  取り合いを防ぐのは `claim` の印で、`doing` は進捗の記録
...
**自分が直したコードを画面で確かめるときは、`TSUKUMO_WORKTREE=0` を付けて自分の作業先で起こす。**
付けないと tsukumo はそこからさらに新しい worktree を切り、**まだコミットしていない変更が入って
いない**作業先で claude が動く（見ているのが自分の直したものではなくなる）。ブラウザ側を直した
なら、起こす前に自分の作業先で `bun run build` を1回打つ（成果物は作業先ごとに別）。
