...
| ## 会話の駆動             | セッション / セッション駆動 / プラン / 利用上限 / コンテキストの内訳 / ターン / ターンの失敗 / 背景のタスク / speak ツール / report ツール / remember ツール / forget ツール / keep ツール / index ツール / recall ツール / recall_episode ツール / 覚えたこと / あらすじ / 最近の話題 / 雑談のアーカイブ / 作業記憶 / 定着 / エピソード索引 / 圧縮の区切り / 許可プロンプト / transcript / セッションスラッグ / 発話 / 見直し / 見直しの段 / 提案 / usage_review_stage ツール / usage_review_result ツール / 見送り / 前回の見直し / diary ツール |
| ## 表示                   | ビュー / 画面 / キャラクター画面 / メインビュー / キャラビュー / ビューサーバ / 立ち絵 / 顔 / プロフィールの札 / ひとことプロフィール / 利用者の呼び名 / ミニ立ち絵の呼び名 / 吹き出し / セリフのログ / セリフ / 詳細 / 最終レポート / 中間レポート / お願い / 選択肢の比較 / 表情 / 衣装 / 書いている / 覆い / いまの作業 / 依頼の手順 / 成果 / 成果の振り返り / 日記 / 日記帳 / しおり / 灯りの暦 / 卒業 / 節目 / キャラクター定義 / フォールバック                                                                                              |
| ## Claude Code 側の仕組み | hook / 状態ファイル / statusline / output style                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ## 通信（移行後）         | プロトコル / アダプタ / フレーム / コマンド / 契約 / 手続き / セッションの姿 / fake driver / 疑似セッション                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- **定義**: claude.ai の契約で使える量の枠（5時間枠・7日間枠など）が、近い・達した、という状態と、
  いつ戻るか。SDK の `rate_limit_event` が運ぶ
- **注記**: 入力欄の経過時間の行に出す（`docs/display.md` 4.2「入力欄」）。**使用率は出さない**
  （単位が型定義に無い）。API キーで使っているときは届かない。**プランとは別のもの**（プランは
  契約の段、利用上限はその段で決まる枠のいまの残り具合）
- **避ける言い方**: レートリミット、クォータ、制限
...
- **注記**: 1つのターンの中に、複数の `speak` の呼び出しと、複数のツール実行が入りうる。
  **依頼なしで claude が自分で始めるターンもある**（背景のタスクが終わった知らせを受けて
  続きを報告する。記録を持たない続きのターンの始まり `turn-resumed` として扱い、吹き出しの
  セリフと表情は持ち越し、メインビューでは元の依頼のやり取りに積む。`docs/screen-design.md` 13.9「背景のタスク」）
...
- **注記**: 理由は型の決まった値だけで、SDK の自由文（`result` の `errors`）は含めない。
  メインビューのやり取りの末尾と、入力欄の経過時間の行（「所要」が「失敗」になる）に出る。
  **API のエラーが届いただけでは失敗ではない**（本体が立て直して続けることがある）。呼び直しの
  途中の状態（再試行中）は一般の概念なので用語にしない（`SessionState.apiTrouble`）
- **避ける言い方**: エラー終了、異常終了（中断まで含むように読める）
...
- **注記**: **雑談モードのときだけ載る**（`docs/design.md` 7.1）。何を書いてよいかの判断は
  モデル側の条（`src/server/chat/core/chat-manner.ts`）が持ち、tsukumo 側は書く場所と上限だけを持つ
...
- **注記**: **雑談モードのときだけ載る**（`remember` と同じ）。**指せるのはそのターンだけ**で、
  ...
  いつ立てるかの判断はモデル側の条（`src/server/chat/core/chat-manner.ts`）が持つ
- **注記**: **やめると決まった**（2026-09-25）。窓から溢れた会話はすべて下の「定着」で
  エピソードに畳まれ、大事さはエピソードの `weight` が持つ。ツールと `kept.jsonl` を読む
  コードが消えたら、この項も消す（`docs/chat-mode.md` 4.9「窓から溢れた会話は定着で畳む」）
...
- **定義**: `remember` と同じプロセス内の MCP サーバで提供するツール。引数は**その日の見出し
  1行**で、**戻り値は `"ok"` だけ**。`index.jsonl` に1行足す
- **注記**: **雑談モードのときだけ載る**（`remember` と同じ）。**書けるのは1ターンに1行、
  120文字まで**で、**見出しが付くのは書いた日**（前の日には付け直せない）。何を書いてよいかの
  判断はモデル側の条（`src/server/chat/core/chat-manner.ts`）が持つ
- **注記**: **やめると決まった**（2026-09-25）。索引を書くのは下の「定着」に移り、形も
  下の「エピソード索引」に替わる。ツールが消えたら、この項も消す
...
- **注記**: **雑談モードのときだけ載る**（`remember` と同じ）。**戻り値が `"ok"` だけではない
  ツール**の1つで、返すのは**その会話自身の過去の目次**だけ（`docs/chat-mode.md` 4.9「古い雑談は
  索引を引いて思い出す」）。**一覧は 2 KiB まで・1ターンに2回まで**（2026-09-25 に2段階へ組み替えた。
  それまでは当たった日の逐語を 8 KiB まで返していた）
...
- **定義**: `recall` と同じ MCP サーバで提供するツール。引数は**一覧で見たエピソードの `id`**で、
  **戻り値はその1件の範囲の雑談そのもの**（アーカイブの逐語を古いほうから 8 KiB まで。知らない
  `id` なら短い一言だけ）
- **注記**: **雑談モードのときだけ載る**。**開けるのは1ターンに2件まで**。開いたことは
  `recalled.jsonl` に残り、そのエピソードが忘れにくくなる（下の「エピソード索引」）。引いた逐語は
  画面に出さない
- **避ける言い方**: 詳細取得、fetch、open、読み込みツール
...
- **定義**: 作業記憶より前の雑談の**話の筋**を1つにまとめた文章。**書くのは下の「定着」**で、
  定着のたびに前のあらすじと畳んだエピソードから書き直される（上書き、8 KiB まで）
- **注記**: 持つのは**新しいセッションへ記憶を渡すため**で、載せ直すのは雑談のセッションの
  `systemPrompt`。載せるのは**新規に起こしたとき**と**`/clear` を見たあと**の2つで、それ以外の
  `resume` には載せない（claude 側の文脈に会話が既にある）。どちらかは**ファイルの1行目の印**が持つ。
  **「別の場所に複製しない」に対して認められた例外の1つ**（残る2つは下の「雑談のアーカイブ」と
  「エピソード索引」。`docs/chat-mode.md` 4.9、形と上限は `docs/design.md` 7章）
- **避ける言い方**: 雑談の要約（2026-09-25 まで）、要約の写し、コンパクト、圧縮ログ、会話の履歴
...
- **注記**: 更新されるのは定着のときだけ。**画面へ運ぶのは見出しだけ**で、あらすじの本文は運ばない
...
- **注記**: **「別の場所に複製しない」に認められた例外の2つ目**（3つ目は下の「エピソード索引」。範囲は
...
- **注記**: **この 64 KiB（下の「作業記憶」）から溢れた会話は、下の「定着」でエピソードに畳まれ**、
  `recall` → `recall_episode` で引いたときだけ戻る（`docs/chat-mode.md` 4.9）
- **避ける言い方**: バックアップ、会話ログの保存、履歴ファイル

### 作業記憶

- **英語識別子（予定）**: `recent`（容量の表の `recentBytes`、読む口の `readRecent`）
- **定義**: 雑談のアーカイブの**新しいほうから 64 KiB** の会話。新しい雑談のセッションを起こすとき、
  逐語のまま `systemPrompt` に載せる（`docs/chat-mode.md` 4.9「直近の会話は逐語のまま読み戻す」）
- **注記**: 画面の 100 ターンより広く、**画面で見た話を忘れることは無い**。ここから溢れた会話は
  下の「定着」が畳む
- **避ける言い方**: 短期記憶、直近ログ、窓（文中で「窓」と略すのはよい）

### 定着

- **英語識別子（予定）**: `consolidation`（関数・型は `chatConsolidation…`）
- **定義**: 作業記憶から溢れた未定着の雑談を、背景の**使い捨ての `query()`** に渡して、
  下の「エピソード索引」の行と、書き直した「あらすじ」と、最近の話題の見出しを返させ、書くこと
  （`docs/chat-mode.md` 4.9「窓から溢れた会話は定着で畳む」、形は `docs/design.md` 7章
  「定着はどこで走るか」）
- **注記**: **雑談のターンの終わりに、溢れた未定着の行が 8 KiB 溜まったときだけ・同時に1本だけ**
  走る。仕事のときは走らない。失敗したらその回を諦め、行は次の契機で拾い直す
- **避ける言い方**: 圧縮、コンパクト、要約処理、インデックス作成
...
- **注記**: **会話の写しではなく目次**（claude が自分の言葉で書いた見出しと要旨）。逐語は
  アーカイブにしか無い。**「別の場所に複製しない」に認められた例外の3つ目**（2026-09-25 に、
  キャラクターが `index` で書いていた `index.jsonl` の見出しを置き換えた。`docs/chat-mode.md` 4.9、
  形と上限は `docs/design.md` 7章）
- **注記**: **忘却は「消える」ではなく「引きにくくなる」**。古くて一度も思い出していないエピソードは
  一覧の下へ沈むが、はっきりした言葉なら届く。**全文検索の仕組みは持たない**
- **避ける言い方**: 雑談の索引（2026-09-25 まで）、全文検索、インデックス（カタカナ）、検索インデックス、目録
...
- **注記**: 状態は **ふだん（`idle`）/ 見直し中（`running`）/ 結果（`result`）** の3つ。見直し中に
  ...
  打ったかは見ない。結果が届かずにターンが終わる（止めた・失敗したも同じ）とふだんへ戻る
  （`docs/design.md`「見直しのツールと状態」）
- **避ける言い方**: 分析、診断、レビュー（`report` の検査の「差し戻し」と紛れる）
...
- **注記**: **提案の識別子**（`usageProposalKey`）は種類と対象の組で、言い回しが変わっても同じ
  提案だと分かる。種類は `shared` に固定した列挙で、列挙に無い種類は受け付けない。押す口を
  押したときに会話へ送る依頼文は、提案が持たずに `usageProposalRequestText` が組み立てる
- **避ける言い方**: アドバイス、改善案、サジェスト（入力欄の補完の候補と紛れる）
...
- **注記**: **仕事のときだけ載る**。境界で検査し、形の崩れた引数や見送った提案の混じった結果は
  状態を変えずに、理由を `isError` 付きの戻り値で返して呼び直させる（`report` と同じ線）
- **避ける言い方**: 結果ツール、提案ツール
...
- **注記**: **取り消す口は無い**（見本にも無い）。取り消したくなったら、ホームのファイルの
  `keys` から手で1件消す
- **避ける言い方**: 却下、無視、非表示（消えるのは画面からだけでなく、次の見直しの対象からも
  外れるため）
...
- **注記**: **「見直し」の状態（`idle` / `running` / `result`）とは別**——見直しは起こし直すと
  ふだんへ戻るが、こちらはプロセスの再起動でも消えない。持つのは直前の1回だけで、古い結果は
  新しい結果に置き換わる
- **避ける言い方**: 履歴、ログ（並べて選ぶものではなく、直前の1回だけを指すため）
...
- **注記**: **載るのは振り返りの使い捨ての問い合わせだけ**で、会話のセッションには載らない
  （2026-09-25。それまでは会話に仕事・雑談とも載っていた）。しおりの ID がその日の終えたタスクに
  無い・本文が空などは、状態を変えずに理由を `isError` 付きで返して呼び直させる（`report` と
  同じ線）。受け付けたら `"ok"` だけ（`docs/design.md`「日記の受け取りと保存」）
- **避ける言い方**: 日誌ツール、振り返りツール
...
- **定義**: 左にパックの一覧、右に選んでいるパックの立ち絵・差し色・背景を並べ、画面から直す画面。
  使用中以外のパックも直せ、そこから切り替えられる。新しいパックを作る**作る画面**へ入る口もここ
  （`docs/screen-design.md` 13.6）
- **注記**: **会話の画面と入れ替わる**（重ねない）。入る口も戻る口も全画面の最上部の帯
...
- **注記**: **任意**で、無いパックの区切りは呼び名を付けず「」だけになる。**キャラクターの言葉
  なのでコードに持たない**（`CLAUDE.md` 原則4）。セリフの中の呼び方は `persona.md` が決め、
  この欄は読まない
- **避ける言い方**: 二人称、ユーザー名（利用者の名前ではなく呼び方）
...
- **避ける言い方**: ミニの名前（`mini` は素材のファイル名の欄で、紛れる）
...
- **定義**: **そのやり取りで最後の、中間でない本文**。`report` ツールが呼ばれたターンでは
  ...
  `selectLastText`）。ラベル「最終レポート」が出るのは、同じやり取りに中間レポートがあるとき
  だけ（本文が1つしか無いやり取りでは区別にならないため）
- **注記**: **以前は「締めの本文」と呼んでいた。** `report` が呼ばれなかったターンの受け皿
  だけに残る過去の呼び名で、いまは判定を持たない（本文の中身が実況かどうかも問わない）。
  それまで積んでいた実況・英訳・資料の判定と、その経緯は `docs/history/decision.md` が正典
- **避ける言い方**: 結論、まとめ、サマリー（`report` ツールの `conclusion` 引数と紛れる）
...
- **定義**: メインが `report` ツールの引数を書いている間（`includePartialMessages` の断片で
  ...
  「締めのセリフより後ろに伸びる本文」から `report` の引数へ移した——`report` の外の本文は
  画面に出ないため）
...
- **注記**: 日はローカル時刻の0時で切り、コミットの日付（committer date）で決める。規則は
  `docs/requirements.md` 4.11、画面は `docs/screen-design.md` 13.10
- **避ける言い方**: 実績、スコア、統計（数を競わせる語にしない）
...
- **定義**: 成果の画面のボタン（と見開きの「この日を振り返る」）から頼む、その日の日記書き。
  ...
  それまでは `speak` で感想を言ってもらい、同日のうちに会話へ1ターンの依頼を送る形から改めた）
- **注記**: 画面を開いただけでは書かせない。雑談のときも会話のターン中も頼める。会話のメインビュー・
  キャラビューには何も出ない。進みは3段（この日のタスクを読む／日記を書く／いちばんを選ぶ）で、
  状態は `diaryWriting`。書かせる口は `diary-writer.ts`（`docs/design.md`「日記の受け取りと保存」）
- **避ける言い方**: 振り返り（単独で使わない。`/retrospect` スキルの「振り返り」と紛れる）、
  レトロスペクティブ
...
- **定義**: 成果の振り返りでキャラクターが `diary` ツールで書く、ある1日の1ページ。本文・しおり・
  ...
  日付ごと**に残す（`docs/requirements.md` 4.11「日記」）
- **注記**: **同じ日に2回振り返ると段落が書き足される**（しおりは新しいほうに差し替わる）。
  日記はセリフの写しではなく、その日のページとして残る別の成果物。成果の画面の吹き出しに
  いちばん新しい段落が、**日記帳**の見開きに全部の段落が出る
- **避ける言い方**: 日誌、ログ、感想（感想は日記の中身の一部で、残る物の名前ではない）、
  レポート（メインビューのレポートと紛れる）
...
- **注記**: 「つくもの日記帳」は画面の固定の名前で、キャラクターの名前ではない（付喪神の「つくも」。
  パックを替えても変わらない）。開く口は灯りの暦の日・成果の画面の「日記帳で読む」・書き終わりの
  知らせの「日記帳で開く」（`docs/screen-design.md` 13.10）
- **避ける言い方**: アルバム、アーカイブ（雑談のアーカイブと紛れる）、履歴
...
- **注記**: 選ぶのはキャラクター（tsukumo は ID がその日の終えたタスクにあるかだけを確かめる）。
  終えたタスクが無い日の日記には無い。書き足したときは新しいほうに差し替わる
- **避ける言い方**: ハイライト、ベスト、おすすめ（数を競わせる語・推薦の語にしない）
...
- **注記**: **灯り**の段階は4つ（灯りなし `none` / ほのか `faint` / ともる `lit` / 明るい `bright`）で、
  その日のコミットの数だけで決まる（区切りは `docs/requirements.md` 4.11「灯りの段階」）。
  **日記あり**の印は鈴で、灯りとは別に出る
- **避ける言い方**: ヒートマップ、草、カレンダー（単独で。日付の選び方の部品と紛れる）
...
- **注記**: 数えるのは tsukumo で、モデルには選ばせない。**先輩タスク**は長く待っていたタスクの
  愛称で、単独の型は持たない。成果の画面の「小さな驚き」と振り返りの依頼に出る
- **避ける言い方**: 滞留タスク、古いタスク、放置（責める語にしない）
...
- **注記**: 数えるのは tsukumo。成果の画面の「小さな驚き」と振り返りの依頼に出る。前日比・連続
  日数のような「比べる数」は節目に含めない（置かない）
- **避ける言い方**: 実績、トロフィー、達成（数を競わせる語にしない）
...
- **注記**: 中身は機能の `core` へ委ねる数行で、判断を書かない。断る条件（雑談の外・ターン中）は
  手続きの中ではなく、契約の `meta` を見る `rpc-guard.ts` が掛ける
- **避ける言い方**: エンドポイント、ハンドラ、コントローラー（手本の語。tsukumo では「受け手」と言う）
