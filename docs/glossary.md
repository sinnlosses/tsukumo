...
| ## 会話の駆動             | セッション / セッション駆動 / プラン / 利用上限 / コンテキストの内訳 / ターン / ターンの失敗 / 背景のタスク / speak ツール / report ツール / remember ツール / forget ツール / keep ツール / index ツール / recall ツール / 覚えたこと / 雑談の要約 / 最近の話題 / 雑談のアーカイブ / 雑談の索引 / 圧縮の区切り / 許可プロンプト / transcript / セッションスラッグ / 発話 / 見直し / 見直しの段 / 提案 / usage_review_stage ツール / usage_review_result ツール / 見送り / 前回の見直し / diary ツール |
...
- **注記**: **仕事のセッションには常に載り、雑談には載らない**（2026-09-24 に試行から採用へ）。
  ターンは `report` → 締めの `speak` → 「完了」の1行の順で終える（条は
...
- **定義**: `speak` と同じプロセス内の MCP サーバで提供するツール。振り返りのターンで、日記の
  本文（`body`）・表情（`expression`）・しおり（`bookmark`。`taskId` と `reason`）を1回で渡す
- **注記**: **仕事にも雑談にも載る**。書けるのは成果の画面から頼まれた振り返りのターンだけで、
  それ以外のターン・しおりの ID がその日の終えたタスクに無い・本文が空などは、状態を変えずに
  理由を `isError` 付きで返して呼び直させる（`report` と同じ線）。受け付けたら `"ok"` だけ
...
- **定義**: 成果の画面のボタン（と見開きの「この日を振り返る」）から送る1ターンの依頼。tsukumo が
  その日の数・終えたタスク・卒業と節目を数えて依頼文を組み、キャラクターに `diary` ツールで
  その日の**日記**を書いてもらう（2026-09-25。それまでは `speak` で感想を言ってもらっていた）
- **注記**: 画面を開いただけでは送らない。雑談のときも送れる。進みは3段（この日のタスクを読む／
  日記を書く／いちばんを選ぶ）で、状態は `diaryWriting`（`docs/design.md`「日記の受け取りと保存」）
- **注記**: いまのコードの依頼文の関数は `achievementReviewRequestText` で、この表の名前と
  ずれている（「review」は「見直し」と紛れる）。依頼文を作り直すときにこちらへ揃える
