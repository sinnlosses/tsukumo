...
| ## 会話の駆動             | セッション / セッション駆動 / プラン / 利用上限 / コンテキストの内訳 / ターン / ターンの失敗 / 背景のタスク / speak ツール / report ツール / remember ツール / forget ツール / recall ツール / recall_episode ツール / 覚えたこと / あらすじ / 最近の話題 / 雑談のアーカイブ / 作業記憶 / 定着 / エピソード索引 / 圧縮の区切り / 許可プロンプト / transcript / セッションスラッグ / 発話 / 見直し / 見直しの段 / 提案 / usage_review_stage ツール / usage_review_result ツール / 見送り / 前回の見直し / diary ツール |
| ## 表示                   | ビュー / 画面 / キャラクター画面 / メインビュー / キャラビュー / ビューサーバ / 立ち絵 / 顔 / プロフィールの札 / ひとことプロフィール / 利用者の呼び名 / ミニ立ち絵の呼び名 / 吹き出し / セリフのログ / セリフ / 詳細 / 最終レポート / 中間レポート / お願い / 検証結果 / 選択肢の比較 / 表情 / 衣装 / 書いている / 覆い / いまの作業 / 依頼の手順 / 成果 / 成果の振り返り / 日記 / 日記帳 / しおり / 灯りの暦 / 卒業 / 節目 / キャラクター定義 / フォールバック                                                      |
...
- **定義**: tsukumo が `speak` と同じプロセス内の MCP サーバで提供するツール。引数は
  `conclusion`（結論。必須）・`checks`（検証結果。任意）・`body`（本文。任意）・`favor`（お願い。
  任意）で、**戻り値は `"ok"` か、差し戻すときの規約違反だけ**。メインビューには `conclusion` →
  `checks`（検証結果の帯）→ `body` → `favor`（お願いの塊）の順に描く
...
- **定義**: **そのやり取りで最後の、中間でない本文**。`report` ツールが呼ばれたターンでは
  ...
  だけ（本文が1つしか無いやり取りでは区別にならないため）。**いちばん新しいやり取りでは、加えて
  そのやり取りが閉じている（ターンが `running` でなく、背景のタスクも残っていない）ときだけ
  `final` が立つ**（2026-09-26 決定。閉じていないあいだは、次の `report` でいま最後の本文が
  中間レポートへ回るかもしれないため）
...
- **定義**: **そのターンで何をどう確かめたか**（テスト・型検査・手で見たこと）。1項目は
  ...
  （OK / NG / 未確認）の両方で見せる
- **注記**: **検証をしていないターンでは渡さない**（帯も出ない）。確かめなかった・飛ばしたものは
  `unverified` にまとめ、理由は `detail` に書く。`conclusion` と `body` には書かない（規約は
  `src/server/report/core/report-notation.ts` の条1）。規則は `docs/display.md` 4.2 が正典
- **避ける言い方**: テスト結果（型検査や目視も入る）、確認結果
