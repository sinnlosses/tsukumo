...
プロセスがセッションの transcript（JSONL）を追従して描画する。発話は**セリフ**（立ち絵ペインの
吹き出し）と**詳細**（Orca 内のブラウザタブに HTML）に分けて出す。**第一級の環境は Orca**。
...
**transcript の追従・吹き出し・hook 連動の表情切り替えまでが動いている。**
立ち絵・セリフと詳細の分離・詳細ビュー・画面レイアウトは**まだ無い**。
...
- **サイドカー本体の実行時の外部コマンド依存は無し。** 画面レイアウトの組み立てと詳細ビューだけは
  `orca` コマンドに依存する（Orca 前提。無い環境ではその機能を諦めて動作を続ける）。
  **これ以外の依存を増やすときはユーザーの承認を得る**
...
bun run start <transcript>    # ローカル実行
```
...
立ち絵＋吹き出しを描く。詳細は HTML にして Orca 内のブラウザタブへ回す。
**tsukumo から Claude Code へ戻る経路は無い**（どちらもファイル経由の片方向）。
