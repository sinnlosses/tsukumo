...
- **`main` へ入れるのは頼まれたときだけ。** 入れるときは `main` を出している作業ツリーを指して
  `git -C <本体> merge <いまの枝>` を走らせる（`main` はそこで checkout 済みなので、
...
- ブランチ: 自分で切らない。いま居るブランチへそのままコミットする（`/next-task` の手順3は
  何もしない。手順8のマージとブランチの削除もしない）
...
- **取り合いを防ぐのは `doing`。** 着手のときに `doing` へ書き換え、`/next-task` が `doing` の
  タスクで止まるのはそのため
