...
**セッションごとに作業ツリーが分かれている。** tsukumo は起こすたびに
`.git/tsukumo/worktree/<時刻>` を切り、claude はそこで `tsukumo/<時刻>` ブランチを動かす
（`docs/architecture.md`「worktree でセッションを分ける」）。**自分でブランチを切らない**
——切り替えても分離は増えず、いま居るブランチがそのまま1セッションぶんの入れ物になっている。
コミットはそこへ積み、**本体（切り出し元の作業ツリー）へ入れるのは1タスクごとに `finish`**
（「## タスク運用」）。**本体は誰も書かない場所**で、そこを保つからマージが「汚れていない相手へ
のマージ」で済む。push は明示的に頼まれたときだけ。

**自分の worktree の中は自分しか書かない。** 他のセッションの未コミットの変更が `git status` に
出ることはもう無いので、`git checkout <file>` / `git restore <file>` で戻してよい
（2026-09-22 まで禁じていたのは作業ツリーが1つだったため）。残る作法は3つ:

- **コミットは `git add -A` を使わず、触ったファイルを個別に足す。** 切った先には
  `node_modules` と `characters/local` の symlink が張ってあり、`.gitignore` の行は末尾が `/`
  なので**symlink には当たらず `?? node_modules` として出続ける**（一括で足すと入ってしまう）
- **本体の作業ツリーに `merge` / `reset` / `branch -d` を走らせない。** 本体へ入れるのは
  `finish` の仕事で、衝突したときに本体を戻すところまで向こうが持っている
- **`.git/tsukumo/` の下（他のセッションの worktree と着手の印）に触らない**

## タスク運用

- 検証コマンド: `bun run check`（変更後は必ずこれを通す。受け入れ判定に使う）
- 整形コマンド: `bun run format`
- ブランチ: 自分で切らない。tsukumo が起動時に切った worktree の `tsukumo/<時刻>` ブランチへ
  そのままコミットする（`/next-task` の手順3は何もしない。手順8のマージとブランチの削除も
  しない——本体へ入れるのは下の `finish`）
...
**着手と完了で tsukumo のツールを1回ずつ呼ぶ**（どちらも `mcp__tsukumo__` の下にある。
仕組みは `docs/architecture.md`「worktree でセッションを分ける」）:

- **手順4（`doing` に書き換えるのと同じ時点）で `claim` を呼ぶ。** 取れなかったら**着手しない**
  ——別のセッションが取っているので、返ってきた pid と作業先を添えて報告し、そのタスクは選び直す
- **手順7（`done` にしてコミットしたあと）で `finish` を呼ぶ。** 着手の印を返し、このセッションの
  コミットを本体へ入れる。**止まったら次のタスクへ進まない**（理由と次の手が返り、同じ文面が
  サイドバーの「セッション情報」にも出る）
- **ツールが無いときはそのまま進めてよい**（git リポジトリでないところで起こしたとき）

**2つのタスクを並行して進めてよい**（触る層が重なっていても選んでよい。セッションごとに作業ツリーが
分かれているので、相手の足元のファイルは動かない）:

- **`doing` は進捗の記録で、排他の根拠ではない**（取り合いを防ぐのは `claim` の印）。
  `/next-task` が `doing` で止まるのは、前のセッションが途中で落ちた跡を人に見せるため
- **検証コマンドは並行して打ってよい**（自分の worktree で走るので、相手の作業中の変更を拾わない）
...
- `develop/tasks.json` も自分のブランチで書き換え、`finish` のマージで本体へ入る。**同じタスクを
  2つのセッションが取ることは `claim` が防ぐ**ので、同じ行がぶつかることは無い
