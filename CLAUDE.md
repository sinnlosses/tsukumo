...
- **原則3**: ホスト（ターミナル環境）・外部コマンド・OSに依存するものは
  **`src/server/adapter/` の1ファイルに閉じ込める**（1ファイル = 1つの境界）。**Agent SDK だけは
  1つの境界が1ファイルに収まらないので、`src/server/adapter/` 直下の `sdk-` で始まるファイルに
  閉じ込める**
...
- **`main` へ入れるまでが開発のひと区切り。** タスクの1サイクル（「## タスク運用」の手順3・5）に
  限らず、会話で頼まれた作業も、検証（`bun run check`、描画に関わるなら目視も）が済んだら
  自分の枝にコミットして `main` へ送る。**送ってよいか聞いて止まらない**——未コミットのまま
  待つと、作業ツリーを後から使うセッションが `git restore` で捨てうる（2026-09-23 に実際に
  9ファイルぶんが痕跡なく消えた）。送るときは `main` を出している作業ツリーを指して
  `git -C <本体> merge --ff-only <いまの枝>` を走らせる（`main` はそこで checkout 済みなので、
  自分の作業ツリーでは切り替えられない）。**本体が clean なことを先に確かめる**。
  `--ff-only` が落ちたら `git merge main` で取り込み、`bun run check` を通し直してから送り直す
...
- **`main` の作業ツリーで起こしたときは手順3・5とアーカイブのマージをしない**（すでに `main` に
  居るので、コミットがそのまま `main` に入る）
...
- **アーカイブ（`develop/task/` の完了したタスクを `docs/history/tasks.md` へ移すこと）は単独の
  ...
  解くより捨ててやり直すほうが安い。理由は `docs/workflow.md`「アーカイブも `main` へ
  単独で送る」）。
  **捨ててよいのは、自分の作業ツリーの中のまだ `main` に入っていないコミットに限る**
  （`main` に入ったコミットは「## Git運用」の「自分の足元だけを触る」の対象で、他の
  作業ツリーへ `reset` はしない）
