...
**タスクは作業ブランチを切って進める**（`/next-task` の既定に従う。`feature/T-<タスクID>` を
`main` から切り、終わったら `main` へマージしてブランチを消す）。**手順の正典は
`~/.claude/skills/task-workflow/WORKFLOW.md`「ファイル配置と CLAUDE.md」のブランチ運用**で、
ここには二重に書かない。「## タスク運用」節の `- ブランチ:` 行が `既定` なのはそのため
（2026-09-16 にこの運用へ変えた。それまでは main に直接コミットしていた）。

**タスク以外のコミットは `main` へ直接でよい**（`/plan-tasks` の登録、ドキュメントの棚卸しなど。
どのスキルもブランチを切らない）。push は明示的に頼まれたときだけ。
