// **変更前の画面を、作業ツリーを1つも動かさずに起こす**道具。名指ししたコミットを /tmp へ取り出し、
...
// 使うので、**本物の index も作業ツリーも読むだけ**で済む。
//
// 使い方:
...
//
// 起こしたものは**自分では止まらない**。撮り終えたら `stop.ts --port` で止める
...
// 起こす理由が無く、API も使わない。**ホームも /tmp に切る**ので、利用者の `~/.tsukumo/`
...
/**
 * 名指しのコミットを /tmp のツリーへ取り出す。**一時 index（`GIT_INDEX_FILE`）を使う**ので、
