...
**`develop/direction.md` には T-069 の結論から出た実装3件がある**（Idiomorph の同梱・ブラウザ側 JS の
切り出しと `bun build`・CSS の分割）。次のセッションは他の作業より先に `/plan-tasks` でタスク化する。
2026-09-12 に届いた指示は T-058〜T-081 に起こし、`docs/history/direction.md` へ移した。
...
- **T-070（sonnet、着手可能になった）**: サイドバーの区画を差し替えると内側のスクロール位置が
  先頭に戻る件。**T-069 の結論で直し方が決まった**（Idiomorph で morph する）。本文を
  差し替えるか、Idiomorph 同梱の作業に統合する
...
- **T-079（sonnet、着手可能になった）**: 段落・表のセル・箇条書きの中の HTML がタグのまま出る件。
  `renderInline` が `escapeHtml` を通すのが原因で、許可リスト（`sanitizeReportHtml`）には
  繋がっていない。**T-069 が「自前のレンダラを保つ」と決めたので、自前で足す前提で進めてよい**
