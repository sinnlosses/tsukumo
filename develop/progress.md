...
- **T-084（sonnet、着手可能になった）**: 残りのブラウザ側 JS をすべて `.ts` へ移す。
  **移す先の入口は `src/browser/main.ts`**（T-083 が作った。1行足すごとに1本増える）
- **T-085（sonnet、着手可能になった）**: CSS の `STYLE` 定数を領域ごとの `.css` に割り、同じ工程でまとめる
