...
- **`lib/` と `utils/` の基準（T-277）で実際に動くのは1件だけ**——`src/shared/image-data-url.ts` → `src/shared/lib/` のみで、**`utils/` に入るものは0件**（`shared` / `server/core` / `server/adapter` の平置きは全ファイルが用語集に載る語を名乗るため）。**ユーザーの指示は「1つ1つのファイルの責務が明確になって良い」**だったので、**この結果が期待と合っているかを確かめてから T-278 に進むほうがよい**。合っていないなら、手順1（用語集に載る語は平置き）を緩めるか、`lib/` の線を「技術に依存するか」から広げることになる
