...
- **書き上げる演出が長い（T-258 の副作用）。** 確定した本文の全体に筆が通るようになった分、1233文字の疑似セッションで**演出が約39.9秒続く**（測ったのは `long-report` の場面）。`docs/requirements.md` 4.3 が「塊が増えて出ている時間が数十秒に伸びる」と予告していたとおりで、**速さ（文字1つ 40ms・塊ごとの上限 8000ms）の調整は別の判断**として手を付けていない。長すぎると感じるかはユーザーが決める
