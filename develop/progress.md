...
`button` / `input` / `select` / `textarea` はブラウザの既定では親の書体を継がないので、`theme.css` の `body` 直後の1か所で継がせる形にし、機能の CSS 12ファイルの40か所を消した。大きさ・行の高さ・太さも継ぐ `font: inherit` を選んだのは、大きさを上書きしていない部品が行の高さ（本文の 1.75）まで親から継いでいたため。`docs/design.md` 13.3 に「機能の CSS には書かない」を足した。
