...
最終更新: 2026-09-16（T-126 で details が機能しないのを直した。T-132 でパックを切り替えても立ち絵が変わらないのを直した。同日 `/plan-tasks` で
...
移行ではない23件。**`loopable: "Y"` が13件、`"N"` が10件**（2026-09-15 に判定の基準を
見直した。`docs/workflow.md`「`loopable` の判定」）。`"Y"` のうち依存が解けているのは
**T-046 / T-121 / T-122 / T-124 / T-126 / T-128 / T-133 / T-134 / T-135 / T-140 の10件**で、これは
