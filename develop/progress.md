...
- **立ち絵の `working` は二役**（`speak` で選べる表情であり、ツールが動くと自動で被さる状態
  でもある）。**T-167 のクールダウン（4000ms）が覆うのは短い隙間だけ**で、
  実測ではターン内の隙間の中央値が 3.5〜11.6 秒あり、往復は残る（2026-09-16 に測り直し、
  **現状のまま許容すると決めた**。遅延を上げるとツールの実行時間の中央値が 1.3〜1.9 秒しか
  ないため `working` ごと消える）。二役であること自体も変わっていない。**クールダウン中の `speak` の表情は無視される**（明けてから出る）
