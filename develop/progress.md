...
「セッション情報」の見出しを外してタスクの区画と同じ枠の仕切り線を借りる形にし、キャラクターと
セッションの2つの `<select>` を小さなラベル付きで横に並べた（760px 以下は縦積みに戻す）。顔は
帯とサイドバーの両方が読むので `src/browser/components/character-face.tsx` へ上げて共有した。
