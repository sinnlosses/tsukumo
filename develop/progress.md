...
- **T-315 / T-316 / T-317（雑談モードを軽くする3件）は `develop/tasks.json` から消えている。**
  2026-09-22 に別のセッションが作業ツリーで消したもので、`docs/history/direction.md` には
  登録した記録だけが残っている。**番号は再利用しない**（新しいものは T-318 から）。
  雑談のモデル・effort・`CLAUDE.md` の載せ方を軽くする話が要るなら、起こし直す
