...
**まず `/plan-tasks` を実行する。** `develop/direction.md` に PoC を触っての感想が入っており、
`docs/requirements.md` 4.2「吹き出し」の要件自体が変わりうる指摘を含む。タスク化の前に
「二重表示の何が厳しいのか」をユーザーに確認すること。

- **T-002**: 立ち絵の素材方針の決定とサンプル1体（`opus` / **ユーザーとの合意が必要**）
- T-004: 画像プロトコルで立ち絵を表示、使えない環境はテキストへ — T-002 の後
- T-006: statusline 併走（案B）— 依存は解決済み。**優先度の見直しが要る**: statusline は
  入力ペインの中にあるので、`develop/direction.md` にある「二重表示が厳しい」への構造的な解に
  なりうる（案Cの弱点を案Bが埋める）

**T-002 はユーザーとの合意を含むので `/loop /next-task` の自動進行に載せない。**
`develop/direction.md` に中身があるうちは `/next-task` が着手を拒むので、そこも自動進行に載せない。
