...
最終更新: 2026-09-13（同日 T-105 で立ち絵の動きに何を作るかを決め、未決事項を1つ閉じた
...
| T-107 決めた4つの立ち絵の動きを実装する          | sonnet     | T-106        | 待ち |

**T-101 は T-106 待ちにしてある**（2026-09-13 の `/plan-tasks`）。段8 がキャラクター切り替えの
`<select>` をサイドバーに足す予定なので、**置き場所を先に決めないと、足してから移す手戻りになる**。

**T-107 も T-106 待ち**（「固定」の切り替えを画面のどこに出すかがそこで決まるため）。
T-105 は 2026-09-13 に完了し、未決事項「立ち絵の動きに何を作るか」は閉じた。
