...
最終更新: 2026-09-13（同日 T-078 で移行の段9（セッションの復元）を終え、残る段は段8だけになった。
...
| T-107 決めた4つの立ち絵の動きを実装する          | sonnet     | （済みのみ） | 可   |

**残り4件はすべて `loopable: "N"`** なので `/loop` では進まない（自分で `/next-task` を呼ぶ）。
T-046 と T-076 は完了条件がユーザーの感想そのものを求めており、T-101 はスパイクが失敗したときに
押し切らず止まる設計、T-107 は「動きが気持ちいいか」が機能の価値そのもの。
