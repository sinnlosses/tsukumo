...
最終更新: 2026-09-15（T-117 で質問を1問1答にし、入力欄の領域が縦にも横にもスクロールしなくなった。
...
- **T-118 の前提が T-117 で変わった。** 本文が根拠にしている「答え待ちの箱の中身が 7px
  はみ出して『送る』ボタンの右端が切れている」は、**その「送る」ボタンごと撤去した**ので
  再現しない。T-117 後の実測では `.layout-dispatch` の `scrollW 669 = clientW 669` で
  横のはみ出しは 0px。**残るのは `.layout-region` の `overflow-x` が `auto` に計算される件**
  （`overflow-y: auto` しか書いていないのに、CSS の規則で横も `auto` になる）で、
  着手するときは本文の症状ではなくそこを見る
