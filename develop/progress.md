...
- **`develop/direction.md` の作業ツリー側が壊れている（2026-09-21）。** `## エージェントの
ドラフト` の本文が、行頭の字下げとバックティックごと崩れた形に書き換わっている
  （`1.` の項目の先頭の文字が欠け、`` `turn.tsx` `` が `` turn.tsx` `` になるなど）。**コミット済みの
  HEAD は無事**なので、壊れているのは未コミットの作業ツリーだけ。エディタの自動整形が
  疑わしい。**これのせいで `bun run check` の `format:check` が落ちる。** 直すとユーザーが
  同ファイルに書いた指示3行を巻き込むので、**こちらからは戻していない**
