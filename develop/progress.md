...
最終更新: 2026-09-14（T-101 で移行の段8を終え、**描く層をブラウザ側へ移す移行が全段完了した**。
...
**移行は全段完了した。** 残り2件はどちらも `loopable: "N"` の「決める」タスクで、完了条件が
**ユーザーの感想そのもの**を求めている（T-046 は箱を使ってみた困りごと、T-076 は待ち時間の何が
つらいか）。どちらも実装は含まない。
