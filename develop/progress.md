...
最終更新: 2026-09-13（同日 T-107 で立ち絵が動くようになった（呼吸・待機中の移動・完了の反応・失敗でびくっ）。
...
| T-101 段8: キャラクターパックと切り替え          | opus       | （済みのみ） | 可   |

**残り3件はすべて `loopable: "N"`** なので `/loop` では進まない（自分で `/next-task` を呼ぶ）。
T-046 と T-076 は完了条件がユーザーの感想そのものを求めており、T-101 はスパイクが失敗したときに
押し切らず止まる設計。**T-101（段8）が終わると移行は全段が完了する。**
