...
- **雑談の記憶の方針（T-261）は、Q1 の承認が出たので動かせる。** 2026-09-21 に着手して
  「生のログを `systemPrompt` に載せるのは規約の例外に収まらない」と判定し、いったん
  ユーザーに預けた。**同日「Q1は問題ないよ」で承認され、`loopable` を `"Y"` に戻した。**
  覆す先は 4.9 と `docs/design.md` 9章と `docs/coding-standards.md` の表と
  `src/server/adapter/chat-archive.ts` 冒頭。残る Q4（表の書き換え方）と Q6（読み戻す量）は
  タスク自身が決める。経緯と案は T-261 の本文にある
- **persona.md の書き戻し先（T-266）が正典で1つに決まっていない。** `docs/design.md` 7.1 は
  「書き込み先は `~/.tsukumo/characters/<name>/` の1箇所だけ」と言うが、人格の正典は同梱の
  `characters/<name>/persona.md` にもある。**同梱側へ書くとリポジトリの作業ツリーが汚れる**ので、
  着手時にここを先に決める
