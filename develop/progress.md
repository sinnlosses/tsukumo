...
- **キャラビューの中の配置**のうち、**吹き出しの縦位置は T-066 で決着**（最新の下端を領域の中心に
  固定）。残っているのは立ち絵と吹き出しの横並びの比率と、下半分の空きをこのままにするか
