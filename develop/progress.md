...
最終更新: 2026-09-12（同日 `/plan-tasks` で届いた指示10件を T-072〜T-081 に起こし、HTML 描画のライブラリ（React）の検討は既存の T-069 に統合した。T-071 でキャラビューを立ち絵主役の形に作り直した。同日 `/plan-tasks` で描画の技術を T-069、サイドバーのスクロール位置が戻る件を T-070、
...
**`develop/direction.md` は空**（2026-09-12 に届いた指示は T-058〜T-081 に起こし、`docs/history/direction.md` へ移した）。
...
- **T-072（sonnet、着手可能）**: 入力欄を Enter で改行・Command+Enter で送信に入れ替える。
  補完候補が開いている間の Enter は確定だけにし、**T-051 の決定を覆す**
...
- **T-079（sonnet、T-069 待ち）**: 段落・表のセル・箇条書きの中の HTML がタグのまま出る件。
  `renderInline` が `escapeHtml` を通すのが原因で、許可リスト（`sanitizeReportHtml`）には
  繋がっていない
- **T-080（opus、T-069 待ち）**: ドメイン駆動・フレームワーク非依存への作り直しを決める。
  原則2・原則3 と既に方向が同じなので、**何に困っているかの特定**から始める。実装はしない
- **T-081（sonnet、着手可能）**: 既定のモデルを Opus・effort `high` にする

T-046 / T-058 / T-059 / T-064 / T-069 / T-076 / T-077 / T-080 はいずれもユーザーがいる
