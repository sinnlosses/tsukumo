...
- **立ち絵の `working` は二役**（`speak` で選べる表情であり、ツールが動くと自動で被さる状態
  でもある）。**T-167 のクールダウン（4000ms）が覆うのは短い隙間だけ**で、実測ではターン内の
  隙間の中央値が 3.5〜11.6 秒あり、往復は残る。**パラメータでは解けない**（遅延を上げると
  ツールの実行時間の中央値が 1.3〜1.9 秒しかないため `working` ごと消える）。**2026-09-16 に
  「往復は許容する」から方針を変え、別の軸で解くことにした**（T-173。ツール実行中は吹き出しにも
  作業中の一言を出し、表情と吹き出しを揃える）。**T-173 より前に書かれた「往復は許容」の記述は
  もう効いていない。** なお**クールダウン中の `speak` の表情は無視される**（明けてから出る）
