...
最終更新: 2026-09-12（`/plan-tasks` で TUI の HTML タグの件を T-058 / T-059 に、ポートの衝突を T-060 に、サイドバーのスクロール範囲を T-061 に、レポートの見やすさを T-062 / T-063 に起こした。承認を得て T-042（旧経路の撤去）と T-048（グローバル導入）を完了。同日 `/loop /next-task` で T-050〜T-057 の8件を完了し、done 10件を
...
**`develop/direction.md` は空**（2026-09-12 に届いた指示は T-058〜T-063 に起こし、`docs/history/direction.md` へ移した）。
...
- **T-061（sonnet、着手可能）**: サイドバーの領域自体のスクロールを切り、3区画をそれぞれ内側で
  スクロールさせる。**目視が要るので `/loop` には載せない**（実装だけ委譲してよい）
...
T-046 / T-058 / T-059 はいずれもユーザーがいるセッションで（委譲せず、`/loop` に載せない）。
...
- **`develop/tasks.json` に無いタスクID（T-001〜T-063 のうち残っている9件以外）は
