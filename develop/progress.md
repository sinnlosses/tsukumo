...
**T-109 / T-110 / T-111 は 2026-09-14 の指示から起こした**（切り替えと復元まわりの使い勝手）。
3件とも現物で再現を確認してある。T-109 は**最初にスパイク**（`resume` したセッションで人格と
`speak` の enum を差し替えられるか）が要り、その結果でユーザーと方式を決める。
