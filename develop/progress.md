...
最終更新: 2026-09-14（`/plan-tasks` で履歴の数・レポートに過程が混ざる件・ミニ立ち絵を
...
移行ではない8件で、**すべて `loopable: "N"`**（`/loop` では進まない。自分で `/next-task` を呼ぶ）。
...
| T-112 レポートの行長の上限から表など広い要素を外す       | opus       | —    | 可   |
...
**T-112 は同日の指示から起こした**（表がつぶれる）。原因は `.detail-block` の
`max-width: 36.36rem` が**子孫すべて**に効くことで、これは `docs/design.md` 13.3
「行長は 42 全角で切る」の実装。**原則を捨てずに例外の線をどこに引くか**が論点になる。
