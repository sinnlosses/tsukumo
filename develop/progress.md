...
**方針が大きく変わった。** tsukumo の目的は「キャラクターと一緒に楽しく仕事をするための
ターミナル環境」で、立ち絵・吹き出し・セリフと詳細の分離はすべてその手段（経緯は
`docs/history/direction.md` 2026-09-09（2））。**T-007 で要件そのものを書き換えるのが起点**。

依存の順に:

- **T-007**: `docs/requirements.md` の目的・スコープ・実行環境の書き換え（`opus` / 依存なし）。
  決定はすべて `docs/history/direction.md` に記録済みなので、新しい方針を発明しないこと
- **T-008**: 出力スタイルにセリフと詳細を書き分ける規約を入れる（`opus` / **ユーザーとの合意と
  グローバル設定の書き換えが要る**）
- T-009: 詳細を HTML にして Orca のブラウザタブで開く — T-008 の後。
  **論点3（会話内容をディスクに書くこと）の承認が先**
- T-010: 吹き出しをセリフだけにする — T-008 の後
- T-011: biim 風レイアウトを `orca terminal split` で組み立てる — T-009・T-010 の後
- T-012: サイドバーに出す情報を決める（`opus` / **ユーザーとの合意が要る**） — T-011 の後
- T-006: statusline 併走が**まだ必要かの判断から**始める — T-012 の後

旧方針から残っているもの:

- **T-002**: 立ち絵の素材方針の決定とサンプル1体（`opus` / **ユーザーとの合意が要る**）
- T-004: 画像プロトコルで立ち絵を表示 — T-002 の後。**どの端末でも画像プロトコルが通っていない**
  という未解決がそのまま残っている

**`opus` のタスク（T-007 / T-008 / T-012 / T-002）はユーザーとの合意を含むので
`/loop /next-task` の自動進行に載せない。** T-007 だけは決定が記録済みなので委譲できる。
