...
- **T-055 / T-056（sonnet、着手可能、依存なし、`/loop` に載せてよい）**: サイドバーの改善の残り。
  T-055 は経過時間を「依頼からターンの終わりまで」にして終わったら止める（`turnFinishedAt` を
  `src/index.ts` の配線で持つ）、T-056 はタスク一覧をバッジ先頭の2列にして見出しに件数を添える
  （`countTaskStatuses` は未使用なので消す）。T-054 は完了。**どちらも `src/view.ts` と
  `test/view.test.ts`「サイドバーの本文」を触るので、同時に進めず1件ずつコミットする。** 目視は同上
