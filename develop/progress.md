...
- **メインビューの「タブを切り替えたら先頭から読ませる」が効いていない**（`.main-turns` に
  overflow が無く、実際にスクロールするのは親の `.layout-region.layout-main`）のドラフトは、
  2026-09-20 の採否で選ばれず `develop/direction.md` に残っている。直し方が2案あって
  どちらもレイアウトの見え方に影響するので、**採るときにユーザーが選ぶ**
