...
- **雑談の記憶の方針（T-261）は、規約を広げるかどうかのユーザーの決定で止まっている。**
  2026-09-21 に着手して判定した結果、**生のログを `~/.tsukumo/chat-archive/` から読み戻して
  `systemPrompt` に載せるのは、いまの「会話内容の扱い」の例外に収まらない**と分かった
  （4.9 と `docs/design.md` 9章と `chat-archive.ts` の3か所が「claude へ戻すこともしない」と
  明記していて、しかもそれがアーカイブを認めたときの根拠。規約自身も「似ているから」で
  広げることを禁じている）。**`loopable` を `"N"` に落として `todo` に戻した。**
  聞くこと（Q1〜Q6）と、Yes になったときの案は T-261 の本文「着手して分かったこと」にある
