...
最終更新: 2026-09-12（`/plan-tasks` でサイドバーの改善を T-054〜T-056 に起こした。同日、吹き出しの分割を
T-053 に、補完の改善を T-050〜T-052 に起こし、progress.md の 2026-09-11 分を `docs/history/progress-archive.md` へ移した。2026-09-11 に方針を全面的に見直して
...
- **`/loop /next-task` に載せられるもの**: **T-050**（sonnet。`/` 補完の候補に CSS を付け、入力欄に
  重ねるポップアップにし、クリックで確定）→ **T-051**（sonnet。前方一致→部分一致・アルファベット順、
  Enter で実行・Tab で補完。T-050 待ち）→ **T-052**（opus。候補に説明を添える。SDK は名前しか
  返さないので frontmatter から引き、足りなければ閉じる。T-050 待ち）。目視はユーザーが後で行い、
...
- **T-048（sonnet、着手可能）**: グローバルへの導入と別プロジェクトでの目視。**承認が要るので
  `/loop` に載せない**
- **T-046（opus）**: Electron の判断。ユーザーの感想が要る。T-042 / T-045 待ち
