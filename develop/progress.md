...
- **`/loop /next-task` に載せられるもの**: **T-051**（sonnet。前方一致→部分一致・アルファベット順、
  Enter で実行・Tab で補完。T-050 は完了）→ **T-052**（opus。候補に説明を添える。SDK は名前しか
  返さないので frontmatter から引き、足りなければ閉じる）。目視はユーザーが後で行い、
