...
最終更新: 2026-09-12（`/plan-tasks` で描画の技術を T-069、サイドバーのスクロール位置が戻る件を T-070、
...
- **T-071（opus、着手可能）**: T-066 で `.balloon-track` を領域の上半分に固定したため、吹き出しが
  使える高さが半分になった件。ユーザーの体感（「使える高さが半分になった」）を聞いたうえで案を選ぶ

T-046 / T-058 / T-059 / T-064 / T-069 / T-071 はいずれもユーザーがいるセッションで
（委譲せず、`/loop` に載せない）。

## 未解決

- **キャラビューの中の配置**: 吹き出しの縦位置は T-066 で一度決着したが、**下半分が空いて使える高さが
  半分になった**ためユーザーから差し戻された（T-071 で見直す）。立ち絵と吹き出しの横並びの比率も
  そこで併せて見る
