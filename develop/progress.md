...
最終更新: 2026-09-16（`/plan-tasks` でアーキテクチャ提案の段1〜段3を T-142〜T-144 に起こした。T-126 で details が機能しないのを直した。T-132 でパックを切り替えても立ち絵が変わらないのを直した。同日 `/plan-tasks` で
...
移行ではない25件。**`loopable: "Y"` が15件、`"N"` が10件**（2026-09-15 に判定の基準を
...
- **Bun を Node へ寄せるかの判断が、どのタスクにも載っていない。** `docs/design.md` は
  「`Bun.*` に寄せない規約を保ち、**箱を決めるとき（T-046）に Node へ寄せるかを判断する**」と書き、
  `docs/research/app-shell.md` も「T-046 は先にこの食い違い（core を Electron の Node で動かす／
  Bun のプロセスを子として起こす）を解く」と名指ししている。ところが 2026-09-16 に T-046 を
  「方針を正典に落とす」へ更新したとき、**解くべき論点からランタイムの件が落ちた**
  （いまの T-046 の論点は文言の3件だけで、「コードを変えない」とも書いてある）。
  T-046 に論点を戻すか、別タスクに切るかが未決（2026-09-16 に気づいた）
