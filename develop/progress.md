...
- **雑談の記憶を「直近は生のまま＋それより前は要約」に変える方針（T-261）は、規約の例外の
  範囲が未判定。** ユーザーの決定は「直近残す量を tsukumo が決め、`/compact` ではなくテキスト
  のまま読み込む」。生のログの出どころは `~/.tsukumo/chat-archive/`（T-255 で実装済み）に
  なる見込みだが、**そこから読み戻して `systemPrompt` に載せるのは新しい動き**で、
  `docs/coding-standards.md`「会話内容の扱い」がいま認めている例外（「別の場所に複製しない」の
  1つだけ）に収まるかは確かめていない。**広げる判断が要るなら押し切らずに止めてユーザーに聞く**
