...
- **`TSUKUMO_CHARACTER` を指定しても `~/.tsukumo/state.json` が上書きされる**（2026-09-21 にT-268 の目視確認で判明）。`src/session-start.ts` の `rememberPack` が駆動を起こすたびに呼ばれるため、使い捨てのパックで実機を起こすと利用者の選択が書き換わる。`src/server/core/character-selection.ts` の「指定があるときは覚えた値を上書きも参照もしない」というコメントと食い違っている。**目視確認のたびに手で戻している**ので、直す価値がある
- **雑談の `/compact` の文面が、雑談のログとアーカイブに `request` として残る**（2026-09-21 に T-268 の作業で気づいた）。`live.prompt(CHAT_COMPACT_COMMAND, [])` で送っているため。T-268 で入れた `promptWithoutRecord` がそのまま使える口なので、直すなら1行の差し替えで済む
