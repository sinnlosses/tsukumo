...
最終更新: 2026-09-13（同日 T-108 で「見た目」の引き出しを作った（色と立ち絵の固定を画面から変えられる）。
...
| T-108 「見た目」の引き出しを作る                 | sonnet     | （済みのみ） | 可   |
| T-107 決めた4つの立ち絵の動きを実装する          | sonnet     | T-108        | 待ち |

**T-106 が完了して詰まりが解けた**（2026-09-13）。キャラクターの `<select>` はサイドバーの
「セッション情報」に入ると決まったので T-101 は着手可になり、立ち絵の固定の切り替えは
「見た目」の引き出しに入ると決まったので、**T-107 は T-108（引き出しの実装）待ちに付け替えた**。
