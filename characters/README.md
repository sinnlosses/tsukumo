...
`chou/` — このリポジトリのために自作した、古い帳面の付喪神（膝くらいの背丈の記録係）。
`assets/chou-design-sheet.png`（設定画）から表情6つと顔（大きな絵の表紙）を切り出したもの。地の除去は仮の仕上がり
（2026-09-25 時点）。
...
- **`background` はキャラビューに敷く背景**（`docs/design.md` 13.8）。**書かなくてよい**——
  ...
  ので、濃さを変えたいときはここを直す）
- **`diaryFont` は日記の本文に効かせる書体**（`docs/screen-design.md` 13.3「例外は日記の本文
  だけ」）。**書かなくてよい**——無ければ端末の明朝体（`--font-serif`）のまま。効くのは成果の
  画面の日記の吹き出しと日記帳の見開きの本文だけ（レポートやセリフの吹き出しの書体は変えない）。
