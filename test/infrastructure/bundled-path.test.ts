...
import { bundledFilePath, resolveBundledDir } from "../../src/infrastructure/bundled-path.ts"

describe("bundledFilePath", () => {
  it("tsukumo 自身の場所（リポジトリのルート）からの相対で解く", () => {
    const path = bundledFilePath("characters", "tsukumo-spirit")

    // このテストファイル自身（test/infrastructure/）から見たリポジトリのルートで組み立て、
    // 一致することを確かめる。src/infrastructure/bundled-path.ts の実装（import.meta.url
    // からの相対）とは別の経路で同じ値を作る。
