...
// 「並べるもの」2「書いたパックが無いとき」）。読むのはこの機能だけなので、フックでない純関数は
// `hooks/` へは置かず機能の直下（`docs/design.md` 2章「フックでない純関数は hooks/ に置かない」）。
//
...
import { portraitAppearance, type PortraitAppearance } from "../../domain/portrait-appearance.ts"
