...
//
// **どの画面でも出す**——`diaryWriting` はセッションの状態（`useSessionSelector`）なので、
// 画面を切り替えても読み続けられる（`diary-notice.tsx` は `main.tsx` の `<Root>` に常駐させる）。
//
...
import { requestDiaryBookOpen } from "../diary-book-request.ts"
