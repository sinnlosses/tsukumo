// 雑談ビュー（<ChatView>。docs/design.md 13.7）の**入口**。**雑談モードの間だけ、メインビューの
...
//
// 雑談ビューが持つ振る舞いと、それぞれの置き場所（決め方はすべて docs/design.md 13.7）:
//
...
// - **届いたばかりのセリフは末尾の行で育つ**（`hooks/use-speech-growth.ts`）。**サーバの契約は
//   変えていない** — 1件まるごと届いたセリフを、ブラウザ側が1文字ずつ出すだけ
// - **返事を待っている間はログの末尾に「...」を出す**（`components/chat-typing.tsx`）
