...
// 内訳は `hooks/use-context-usage.ts` が持ち、見た目は
// `presentational-token-usage-screen.tsx` が持つ（docs/design.md 2章「機能の中を分ける」の
// container / presenter）。**2つの取得が別のフックなのは、出どころが別だから**（記録の集計は
// ファイル、内訳はいま動いているセッションの駆動）。
//
...
export function TokenUsageScreen(): ReactElement {
  return <PresentationalTokenUsageScreen {...useTokenUsage()} contextUsage={useContextUsage()} />
}
