...
/**
 * 図・グラフ1つのフェードに使ってよい時間の上限。**割り当てが長くなっても打ち切る**
 * ——図しか無いレポートで持ち時間を1つの塊が独り占めすると、ただ遅いフェードに見える。
 * 打ち切った分は後ろの塊が前倒しで使う（{@link planReveal} は時間を詰めて並べる）。
 */
...
/**
 * 根の直下の塊を、書く順（文書の順）に並べて時間を割り当てる。**合計は `budgetMs` を超えない**
 * （文字数に比例させると長い本文で読み始めが遅れる。`docs/requirements.md` 4.3
 * 「全体で1.5〜2秒を上限に出し切る」）。
 *
 * 塊の間に隙間は空けない（前の塊が終わった時刻が次の塊の始まり）。
 */
export function planReveal(root: Element, budgetMs: number): readonly RevealBlock[] {
  const shapes = [...root.children].filter(isRevealElement).map(toShape)
  const totalWeight = shapes.reduce((sum, shape) => sum + shape.weight, 0)
  if (totalWeight === 0) {
    return []
  }

  return shapes.reduce<{ blocks: readonly RevealBlock[]; at: number }>(
    (acc, shape) => {
      const share = (budgetMs * shape.weight) / totalWeight
      const endMs = acc.at + (shape.kind === "figure" ? Math.min(share, MAX_FIGURE_FADE_MS) : share)
      return {
        blocks: [
          ...acc.blocks,
