...
// メインビューの中なので、機能の中（この演出のディレクトリ）に置く（`docs/design.md` 2章
...
// 書き始めたときだけそちらへ移る。undefined に戻るのは、まだ一度も
...
/**
 * いまなぞっている画の種別。Z字のどの画かで追従の間合いが変わる（`mini-portrait.tsx`）ので、
 * 「書いているか」を `| undefined` の組み合わせではなくこの1つの印で表す。
 *
 ...
 *
 * `top` / `bottom` はいま書いている帯（Z字の1画。`use-report-reveal.ts`）の上端と下端。
 ...
 *
 * - `writing`: なぞっている最中。`stroke` はどの画かの印で、位置と同じ1つの値の中に持つ
 *   （画ごとに別の口で配ると、位置と種別がずれたフレームができる）
 * - `resting`: 書き終わってその場に残っている。位置は最後の行が終わったところ（帯ではなく
 *   行。帯の右端はその帯でいちばん長い行の右なので、短い行で終わる本文では右へ外れる。
 *   `use-report-reveal.ts`）。次に書き始めるまで消えないので、なぞる画も持たない
 */
...
/**
 * 書いていた筆先を最後に配ったところに残す。演出が終わるときの落とし先で、ふだんは
