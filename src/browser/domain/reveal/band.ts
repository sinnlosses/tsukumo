// レポートを「書き上げていくように見せる」演出で、筆がどこをなぞるかを決める（純粋な計算。
// 位置を測るのは `measure.ts`、いつ出すかは `plan.ts`）。
//
// 塊（トピック）の行をZ字の帯に割り、帯の上の `progress`（0〜1）から筆の居場所を出す。
//
// 帯の右端は、その帯にある行のいちばん右（それまでは塊を囲む枠＝いちばん広い
// 要素の幅だった）。枠でなぞると、箇条書きのように短い行しかない帯では横画の2〜5割しか
...
// 決まる（`plan.ts`）ので、帯へ配り直すときも幅＝その帯の文字数の目安で分ける。
//
...
/** 塊を囲む枠（ビューポート座標）。行が1つも取れない塊の落とし先として使う。 */
...
/** 塊1つぶんの帯。必ず1つ以上あるので、使う側で「無い」を考えなくてよい。 */
...
/**
 * 行の矩形をZ字の2画に割る。切れ目は真ん中の行の下端なので、帯の境目が文字を上下に切らない。
 ...
 * （`use-report-reveal.ts`）——帯の右端はその帯でいちばん長い行の右なので、短い行で終わる
...
/**
 * 縦に重なる矩形を1行にまとめる。行の矩形は行ごと・インラインの箱ごとに取れるので、
...
/**
 * 横画の時間を帯の幅で按分して並べる。幅がぜんぶ 0 の塊（まだレイアウトされていない）は
 * 等分に落とす。
 */
