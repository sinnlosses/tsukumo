...
// （`use-report-reveal.ts` 冒頭）。立ち絵の立つ位置は帯の下端付近の狭い範囲なので、
// 帯が背高でもこの往復は起きない。
//
...
// （`use-report-reveal.ts`）ので、送り続けると読もうとした位置から引き戻してしまう。降りたら
// その演出のあいだは戻らない——読む位置を選んだのは利用者のほうなので、筆のほうが譲る。

/**
 * 筆先を器の上下の縁からこれだけ離して保つ。縁に貼り付くと、次に書かれる行が見えない
 * まま筆だけが進んでいるように見える。
 */
const KEEP_MARGIN_PX = 96

/**
 * ミニ立ち絵が立つ位置の縦の範囲。ビューポート座標（`measure.ts` が測った生の値で、
 ...
 * （`use-report-reveal.ts`）——実際の高さは窓幅で 60〜96px に変わるが
...
/**
 * 筆先を追いかける器を1つ決めて、そこへ送る手を返す。演出のあいだ毎フレーム呼ぶ。
 *
...
export function brushScroller(root: Element): BrushScroller {
  ...
  const follow = (tip: BrushTipRange | undefined): void => {
    ...
    // `tipHeight + KEEP_MARGIN_PX×2` 要る。それより器が低いときは下端（いま書いている足元）を
