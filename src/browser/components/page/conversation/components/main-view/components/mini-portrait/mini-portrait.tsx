...
// 筆先を配るのは演出
// （`domain/reveal/use-report-reveal.ts`）で、演出が掛かるのはいちばん新しいターンの最後の確定レポートが
// 現れたときだけ（`turn.tsx`）。`prefers-reduced-motion: reduce` と過去のターンでは演出自体が
...
// - 書き終わりに行の上へ立っていたので、その上の行の文字を隠していた。残っているあいだは
//   最後の行の下へ降りる（{@link followStyle}）。降りた先の床は {@link MiniPortrait} が
//   本文の末尾に敷く
//
...
export function MiniPortrait(props: MiniPortraitProps): ReactElement | null {
  ...
  return (
    // 本文の末尾に敷く床。書き終わって本文の下へ降りた立ち絵の立つ場所で、これが無いと
    // 足元が器の下端をはみ出して、転がさないと見えない（立ち絵は `position: absolute` なので、
    // 場所を空けられるのは流れの中にいるこちらだけ）。中の立ち絵の置き先はこの床ではなく
    // 本文の入れ物のまま（床は position を持たない。`mini-portrait.module.css`）。
...
/**
 * 追従の間合いを画ごとに変える。横画のあいだは遅れて寄り、斜めの戻りの
 * あいだは筆先に張り付く——戻りは横画の4〜7倍の速さで動くので、同じ間合いのままでは
 * 立ち絵の幅より大きく置いていかれる。長さは `mini-portrait.module.css` の2つの custom
 * property が持つので、ここは class を選ぶだけにする。
 *
...
/**
 * 筆先の右・帯の下端に立たせる（なぞっている帯と同じ高さで、書き進む先の側）。座標は
 ...
 * 止まると文字を隠したまま居座る（画面で出た。立ち絵の高さは行の2〜3本分あり、最後の行が
 ...
 * `transition`）が毎フレーム引き直されるので、横画では少し遅れてばねで寄り、戻りでは
