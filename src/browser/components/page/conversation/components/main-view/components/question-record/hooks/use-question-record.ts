...
// presenter）。質問ごとに、選択肢と答えの突き合わせ・比べた `preview` の一覧を画面に出す形へ
// 畳み、開くまで preview を描かない（開いたかどうか）を持つ。
//
// 自由入力の答えは選択肢のどれとも一致しないので、選択肢の並びの下に別の行として持たせる
// （`docs/display.md` 4.2「許可と質問」）。
//
...
// まま組み立てると潰れたグラフが残る）。質問ごとに独立して開閉するので、開いた索引を
// `Set` で持つ。
//
// `<details onToggle>` は開閉のたびに発火するので、開いたとき（`event.currentTarget.open`）
