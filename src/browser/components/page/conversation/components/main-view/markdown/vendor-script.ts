...
// `src/server/view-server/adapter/vendor-asset.ts`）。その記法が実際に出てきたときだけ読む
// （`docs/display.md` 4.2）ので、`<head>` へ置くのはここが最初に呼ばれた瞬間。
//
...
// （`chart.ts`）の2つだけなので機能の中に置く（`docs/design.md` 2章「その機能しか読まないなら
