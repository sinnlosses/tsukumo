...
// 日本語ラベルにする。読むのは帯（`components/domain/screen-nav/`）だけ——操作子も読みも帯へ集まったので、
 ...
 * 部分一致にしてあるのは、フルネームの形（`claude-opus-4-1` のような値）が実装側の都合で
 * 変わりうるため。まだ届いていない・対応しないときは既定へ畳むので、呼んだ側は
 * 「必ず値がある」型で受け取れる。
 */
