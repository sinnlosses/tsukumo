...
// パックの名前。画面（キャラビュー・サイドバーの `<select>`・キャラクター画面）が読む形で、
...
export type CharacterInfo = {
   ...
   * 持つので、読む側は「あるかどうか」だけを見れば足りる（縮小するのは画面側）。
  ...
  /**
   * 立ち絵と差し色を画面から変えられるか。変えた結果の書き込み先は
   * `~/.tsukumo/characters/<name>/` の1箇所だけで（`docs/design.md` 7.1）、そこに書いた版が
   * 探索の順で起動先の `characters/local` に負けるパックだけが false になる
 ...
 * 一覧の1件（{@link CharacterPackEntry}）はこの形を含むので、選択肢だけを読む部品には
 * 一覧をそのまま渡せる。
 */
 ...
 * - `"revert-to-bundled"`: 同梱のパックを画面で直したもの。ホームの版が消えて同梱の版が一覧に
...
/**
 * 新しく作るパックの名前として受け付ける形か（`docs/design.md` 7.1）。作るときだけは
 ...
 *   経路が生まれない）
 *
 * 表示名（`character.json` の `name`）は別の関数（`characterPack.create` / `characterPack.setProfile` が
