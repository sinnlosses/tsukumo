// トークン消費の記録の形（何にどれだけ使ったかを残すため。それまで
// SDK が渡してくる使用量は届いた時点で捨てていた）。
// ここにあるのは型だけで、書くのは `src/server/token-usage/adapter/token-usage-log.ts`、何をいつ書くかを
...
// （長さは数なので、結果の大きさだけは残せる）。
//
// この線は広げない。 コンテキストの内訳（メモリファイルのパスやスキル名を持つ）はここには
...
/**
 * モデル1つぶんの使用量。SDK の `modelUsage` の1件を写した形で、数だけを持つ
 * （`inputTokens` などの名前も SDK に合わせてある。`sdk.d.ts` のとおり）。
 *
 * 記録に書くときは「前の `result` からの増分」で、SDK から届く累計そのものではない
 * （差分を取るのは `src/server/token-usage/core/token-usage.ts`）。同じ型を累計にも増分にも使うのは、
 * 持っている数の並びが同じだから。
 */
...
/**
 * ターンの中の持ち場。メインループか、サブエージェント（Agent ツール）の中かの2つで、
