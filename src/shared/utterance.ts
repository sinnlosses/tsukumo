...
//
// セリフの主経路は `speak` ツール（src/shared/session-event.ts）で、ここはその補助。
// **`speak` が1度も呼ばれなかったターンだけ**、本文の行頭マーカーをセリフとして拾う
// （docs/requirements.md 4.2。呼び出しは src/shared/session-state.ts の畳み込み）。
 ...
 * 経路になる（判断するのは呼び出し側の `src/shared/session-state.ts`）。コードブロック
