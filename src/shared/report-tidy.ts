...
//
// **落とすのは、落としても意味が変わらない行だけ**で、言い換え・要約・並べ替えはしない。
// 判定は行の丸ごとの一致か、中身が何も無いことだけで、**行の一部を削ることも無い**。
// 直すのに書き直しが要る条（段落を表へ移す・結論を短くする）は検査の段
// （`src/server/core/report-violation.ts`）が差し戻す側で、ここと同じ条は持たない。
//
...
/**
 * 前置き・締めの定型。**内容を持たないことが文面だけで決まる行だけ**を、行ぜんぶとの一致で
 * 落とす（末尾の句点・感嘆符は見ない）。「〜を見てみます」のような**型で当てる判定は置かない**
...
/**
 * 中身の無い見出し。**後ろから1回なめる**ので、小見出しがみな空で落ちた親の見出しも、
