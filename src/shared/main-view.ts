 ...
 * 伸びたときの止め**（`src/browser/components/page/conversation/components/main-view/turn.tsx` の `MAX_REQUEST_HEADING_TEXT_LENGTH`
 ...
 * やり取りの末尾に「失敗で終わった」と理由を出す（`src/browser/components/page/conversation/components/main-view/turn.tsx`）。
 ...
 * 見分けを付けて描くのは `src/browser/components/page/conversation/components/main-view/turn.tsx` の仕事で、判定はここに置く。
 *
 ...
 * 相手を選ぶのにも使う（`src/browser/components/page/conversation/components/main-view/turn.tsx`）。**ラベルを出すかどうかは
 ...
 * 前に振る）。`src/browser/components/page/conversation/components/main-view/turn.tsx` の `<Step>` の `key` に使う。**配列の添字を `key` に
...
export type MainViewTurn = {
   ...
   * （`src/browser/components/page/conversation/components/main-view/turn.tsx`）。
   */
 ...
 *
 * **`tool` の記録も渡す**（ステップの `actions` に入る）が、`src/browser/components/page/conversation/components/main-view/turn.tsx` は
 ...
 * あるかどうかを畳む。**引くのは1箇所だけ**にして、描く側（`src/browser/components/page/conversation/components/main-view/turn.tsx`）が
...
/**
 * そのステップが画面に出す記録の件数。**`src/browser/components/page/conversation/components/main-view/turn.tsx` が描くもの**
