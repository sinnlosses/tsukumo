...
// 1行 = 1セッションで、内訳のうちメッセージ以外（システムプロンプト・ツール定義・
// メモリファイル・スキル・MCP）はセッションの中でほぼ変わらず、変わるのはセッションを
// またいだときだから、ターンごとには積まない。
// ここにあるのは型だけで、書くのは `src/server/context-usage/adapter/context-usage-log.ts`、いつ書くかを
...
// 種類ごとに違うから——同じファイルに混ぜると、広いほうの線が狭いほうにもかかる。
//
...
//   分類1行の数として出るだけで、文面の口は型に無い（ターンごとの記録と同じ手で、
