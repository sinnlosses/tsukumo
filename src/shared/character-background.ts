// キャラビューに敷く背景（`docs/screen-design.md` 13.8）。背景はキャラクターパックの持ち物で、
// `character.json` の `background`（素材のファイル名と覆いの不透明度）から来る。効くのは
// キャラビューだけで、メインビュー・サイドバー・入力欄には敷かない（読む面の地は誰が来ても
// 同じにする。13.1 原則1）。
//
// ここが持つのは「無い」を畳む境界の1つ。定義ファイルも画面から届く画像も外部由来なので、
...
// （`src/browser/domain/appearance-color.ts`）。
...
/**
 * 背景の素材として扱ってよいファイル名か。定義ファイルに書かれた名前も外部由来なので、
...
/**
 * data URL を背景1枚として読む。読めない・受け付けない種類・大きすぎるときは undefined
