...
// 直近の数枚だけが棚（`src/server/session-driver/core/prompt-image-shelf.ts`）に残る（記録の窓から出た依頼の
...
/**
 * 受け付ける形式（メディアタイプ）。API の天井そのもので、`Base64ImageSource.media_type` が
...
/**
 * 控え1枚（デコード後）の上限。記録に残り続けるのは控えだけなので、原寸とは別に、
 * 桁の違う小さな値で切る（雑談の 100 ターンぶん積んでも記録の大きさが暴れない。
 * `docs/requirements.md` 4.10「控えだけを記録に残すので、記録の大きさが上限を持つ」）。
 * 縮めるのはブラウザ側（`src/browser/components/page/conversation/components/dispatch/components/composer/domain/prompt-image.ts`）で、サーバは加工しない。
 */
...
/**
 * 記録に載る1枚。控えと、棚の原寸を指す id の組で、原寸そのものは載らない
 ...
 * 要る経路で、WebSocket のフレームには原寸を載せない（押したときだけ取りに行く）。
 */
...
/**
 * data URL を原寸の画像1枚として読む。読めない・受け付けない種類・大きすぎるときは
