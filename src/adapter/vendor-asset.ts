 ...
 * `test/adapter/vendor-asset.test.ts` が allowlist 側から辿って確かめる）。
 *
 * **mermaid と chart.js は package.json で版を固定している**（`^` を付けない）。素の JavaScript を
 * そのままブラウザへ配っていて、描けるかどうかは目で見るまで分からないため——mermaid の版は
 * `src/core/report-notation.ts` が挙げる図の10種の根拠でもある。highlight.js のテーマだけは
 ...
 * undefined（呼び出し側 = `src/adapter/server.ts` が404にする）。名前は対応表を引くだけで、
