...
// どのファイルを指すかを知っているのはここだけで、`src/shared/vendor-asset.ts` は配る名前と
// Content-Type しか持たない（原則2・原則3。パス解決は外の世界に触る仕事）。
//
// 束ね（`src/server/view-server/adapter/bundle.ts`）には入れない。 mermaid だけで 5.3MB あり、入れると
 ...
 * `test/server/view-server/adapter/vendor-asset.test.ts` が allowlist 側から辿って確かめる）。
 *
 ...
 * highlight.js のほうなので、版がずれると当たらない class が出る。いまは `lowlight` が
 * `~11.11.0` で別の複製を持つが、配っている 11.12.0 のテーマとバイト一致なので当たる
...
/**
 * 配ってよい名前1つを読む。allowlist に無い名前・`node_modules` に無いファイルのときは
 * undefined（呼び出し側 = `src/server/view-server/adapter/server.ts` が404にする）。名前は対応表を引くだけで、
 * 要求されたパスからファイル名を組み立てないので `..` で外へ出る経路が無い。
 */
