...
// 読むのも一覧を作るのも外の世界に触る仕事なので、それは `src/server/character-pack/adapter/character-pack.ts` に
 ...
 * パック」を1つの欄で兼ねないための判別可能な合併型（兼ねていたせいで、モードを切り替えた
 ...
 *
 * 指定があるときは覚えた値を読みに行かない（環境変数は「その回の上書き」なので、前回の値に
