// コンテキストの内訳を記録に残すときの書き口の契約（`core/token-usage.ts` と同じ切り分け）と、
// **セッション1つにつき1行だけ書く係**（{@link ContextUsageRecorder}）。**実際に書くのは
// `src/server/adapter/context-usage-log.ts`**、**いつ呼ぶか**（ターンが終わるたび）を決めるのは
