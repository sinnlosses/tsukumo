 ...
 * でも分類ごとの値が実測で最大2倍ずれる（メモリファイル 12244 → 5730、システムツール
...
/**
 * SDK が返す内訳のうち画面が要る鍵だけを見るスキーマ（外の世界の値なので境界で検証する。
 ...
 * 取れなかった回は「取れない」を返すだけで、例外は外へ出さない。
 *
 ...
 *
 * `skills` は1件ずつの並びではなく、まとめの中の `skillFrontmatter` に入っているので、
