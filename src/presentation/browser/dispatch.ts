...
//   （`Date.now()` を呼ぶのは副作用なので `src/presentation/view.ts` の純粋関数には置けない）。終了時刻が
//   あればそれを終点に固定し、ラベルを「経過」→「所要」に書き換える（60秒未満は `N秒`、
//   以降は `M分SS秒`）
//
// **送信ラベル・経過時間の既定ラベル・Command+Enter の記号は、`src/presentation/view.ts` が初期 HTML に
...
export function bindDispatch(elements: DispatchElements, config: DispatchConfig): void {
  ...
  // 使う（`src/presentation/view.ts` の `dispatchRegionHtml` が組み立てた値と二重に持たない）。
...
/** `encodeTurnStatus`（`src/presentation/view.ts`）が組み立てた JSON を読む。壊れていれば「進行中でない」扱い。 */
