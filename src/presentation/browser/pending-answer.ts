...
//   1問につき1つの文字列しか受け取らない。`src/domain/pending-answer.ts`）

// `src/presentation/view.ts` が答え待ちの箱の HTML に付ける、クラス名（構造上の取り決め。
...
/** `data-answer` 属性（`src/presentation/view.ts` が JSON で埋め込む `{kind:"allow"}` / `{kind:"deny"}`）を読む。 */
