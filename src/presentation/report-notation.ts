...
| 比較・対応・件数 | GFM のテーブル | 3行以上あるときだけ。1〜2行なら文で書く |
...
| 座標のある図・mermaid で描けない図 | <svg viewBox="0 0 240 80"> の中に rect / line / path / text | mermaid で描ける形（流れ・依存・遷移）なら mermaid を使う |
| 手順 | 番号付きリスト | 順番に意味が無いなら箇条書き |
| 発見・候補・ファイルの一覧 | 箇条書き（**ネストは1段まで**） | |
| コード・コマンド・エラー文 | 言語名付きのフェンス | 地の文に混ぜない |
| よそからの引用（ドキュメント・出力・ユーザーの言葉） | <blockquote> | 自分の言葉を目立たせるために使わない |
| 話題の区切り | <hr> | 見出しを立てるほどでない切れ目にだけ |

- **見出しは \`##\` / \`###\` を数個まで。** レポートの見出しに \`#\` は使わない
- **Markdown の引用 \`> \`・ネストしたリスト・画像・水平線 \`---\` は描けない**（段落の文字のまま
  出る）。引用と区切りは上の表のとおり HTML で書く
