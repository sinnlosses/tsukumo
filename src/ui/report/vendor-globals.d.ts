...
declare global {
  /**
   * mermaid（`vendor/mermaid.min.js`）。**`src/ui/report/mermaid-block.tsx` が図の記法を
   * 見つけたときだけ**動的に読み込むので、参照する時点（読み込みの `then` の中）では必ず存在する。
   */
  const mermaid: {
   ...
   * 暗い配色へ寄せるためだけに触る（`src/ui/report/chart-block.tsx`）。
   */
