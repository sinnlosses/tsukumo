...
// 読むのも一覧を作るのも外の世界に触る仕事**なので、それは `src/adapter/character-pack.ts` に
 ...
 * `src/adapter/character-pack.ts` の `CharacterPack` が持つ）ので、選ぶのに要る一片だけを見る。
 */
...
export type InitialCharacterPackOptions<Pack extends NamedCharacterPack> = {
  ...
  /** 前回に画面から選んで覚えたパックの名前を読む（実装は `src/adapter/remembered-character.ts`）。 */
