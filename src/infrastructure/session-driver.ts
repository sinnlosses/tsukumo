...
// 呼ぶのが src/infrastructure/orca-host.ts だけなのと同じ扱い）。SDK の語彙を外へ漏らさないため、外に出す型は
...
import { type Expression, expressionLabel } from "../domain/expression.ts"
import {
  type Answer,
  createPendingAnswerQueue,
  type PendingAsk,
  type PendingAnswerQueue,
} from "../domain/pending-answer.ts"
import {
  type SessionEvent,
 ...
 * （{@link isPermissionMode}）がここを参照する。** `src/presentation/view.ts` の `<select>` は
 * このファイルを import しない（原則3。SDK への依存を session-driver.ts に閉じるため）ので、
 * 画面に出す並び・日本語ラベルは `src/presentation/view.ts` 側に別で持つ（値そのものは同じ5つ）。
 */
 ...
 * 以降セッション中に増減したときは `commands_changed` が押してくる（src/domain/session-event.ts）。
 *
 ...
 * （src/domain/session-event.ts）。受け取り口を1つにしておくと、イベントの流れが1本で済む。
 */
